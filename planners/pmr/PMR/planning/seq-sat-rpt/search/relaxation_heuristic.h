...
public:
    ...
    // Vidal: public access to the propositions
    std::vector<std::vector<Proposition> > get_propositions(){
        return propositions;
    }
    // Vidal: (re)set the goal propositions
    virtual void build_goals(std::vector<std::pair<int, int> > goals);
