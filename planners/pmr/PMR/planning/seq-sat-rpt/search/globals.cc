...
void read_mutexes(istream &in) {
    ...
    for (size_t i = 0; i < num_mutex_groups; ++i) {
        ...
        int num_facts;
        in >> exactly_one_str; // whether it is a exactly one invariant or a regular mutex group
        in >> dir; // direction of computation i.e. if computed forward, use it in backward search
        in >> num_facts;
        ...
        // add it to the exactly_one invariants group
        if (exactly_one_str == "exactly_one")
            g_invariant_exactly_one.push_back(invariant_group);
...
void read_goal(istream &in) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        g_original_goal.push_back(make_pair(var, val)); // Vidal
    }
...
vector<pair<int, int> > g_original_goal; // Vidal
vector<vector<pair<int, int> > > g_invariant_exactly_one; // Vidal
