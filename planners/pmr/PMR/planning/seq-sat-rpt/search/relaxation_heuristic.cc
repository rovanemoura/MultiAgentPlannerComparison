...
// Vidal: Build goal propositions
void RelaxationHeuristic::build_goals(vector<pair<int, int> > goals) {
    goal_propositions.clear();
    for (int i = 0; i < propositions.size(); i++)
        for (int j = 0; j < propositions[i].size(); j++)
            propositions[i][j].is_goal = false;


    for (int i = 0; i < goals.size(); i++) {
        int var = goals[i].first, val = goals[i].second;
        propositions[var][val].is_goal = true;
        goal_propositions.push_back(&propositions[var][val]);
    }
}
