...
int is_var_in_prec_cvar(int lvar, int cvar_index) {
  ...
  switch (gcomp_var[cvar_index].l_operator)
    {
    case UMINUS_OP:
      return is_var_in_prec_cvar(lvar, gcomp_var[cvar_index].first_op);
    case MUL_OP:
    case DIV_OP:
    case MINUS_OP:
    case PLUS_OP:
    ...
    case LESS_THAN_OR_EQUAL_OP:
      return (is_var_in_prec_cvar(lvar, gcomp_var[cvar_index].first_op) || is_var_in_prec_cvar(lvar, gcomp_var[cvar_index].second_op));
    case VARIABLE_OP:
      return (gcomp_var[cvar_index].first_op == lvar);
    case FIX_NUMBER:
      return FALSE;
    default:
      if (DEBUG1) 
	printf ("\n\nwrong cvar [%d] : found operator %d \n\n", cvar_index, gcomp_var[cvar_index].l_operator);      
      return FALSE;
    }
}
...
int is_var_in_eff_cvar(int lvar, int cvar_index) {
  ...
  switch (gcomp_var_effects[cvar_index].l_operator)
    {
    case INCREASE_CONN:
    case DECREASE_CONN:
    case ASSIGN_CONN:
    ...
    default:
      if (DEBUG1) 
	printf ("\n\nwrong cvar [%d] : found operator %d \n\n", cvar_index, gcomp_var_effects[cvar_index].l_operator);      
      return FALSE;
    }
}
...
apply_numeric_effects_of_action (int act_pos, int level)
{
  ...
  if (numeric_effs)
	...
	refresh_cvars_at_start (level);
      ...
      if (GpG.durative_actions_in_domain)
	memcpy(vectlevel[level+1]->numeric->values, vectlevel[level]->numeric->values_after_start, gnum_comp_var*sizeof(float));
...
void eval_comp_var_non_recursive (int cv_index, float *in_vect, float *out_vect, int in_level, int out_level)
{
  ...
  switch (gcomp_var[cv_index].l_operator)
    ...
    case EQUAL_OP:
      ...
      // ho aggiunto fabs(...) xkè può succedere che in_vect[first_op] sia < di in_vect[second_op] (in questo caso
      // si aveva un risultato negativo e il confronto <= MAX_APPROX dava TRUE ...) 
      out_vect[cv_index] =
	(float)(fabs(in_vect[first_op] - in_vect[second_op]) <= MAX_APPROX);
...
void eval_comp_var_non_recursive_effects (int cv_index, float *in_vect, float *out_vect, int in_level, int out_level)
{
  ...
  //versione non ricorsiva della eval_comp_var
  switch (gcomp_var_effects[cv_index].l_operator)
    {
    case INCREASE_OP:
      out_vect[first_op] = in_vect[first_op] + in_vect[second_op];
      break;
...
void propagate_cvars (int level_from, int level_to)
{
  ...
  if(level_from==level_to)
   ...
   for (level = level_from; level < level_to; level = level+1)
     {
	    ...
	    CONVERT_ACTION_TO_VERTEX(GET_ACTION_POSITION_OF_LEVEL (level + 1))->duration_rvals != NULL)
	  {
	    dur_after =
	      get_action_time (vectlevel[level + 1]->action.position,
			       level + 1);
	    if ((fabsf (dur_after - dur_before) > MAX_APPROX)
		&& GpG.temporal_plan)
	      {
		insert_propagation_list (GET_ACTION_OF_LEVEL (level));
...
Bool is_num_prec_satisfied (int cvar, int level)
{
  ...
  satisfied = ((fabsf (vectlevel[level]->numeric->values[cvar] - 1)) < MAX_APPROX);
  ...
  if (satisfied)
    ...
    {
      vectlevel[level]->numeric->values[cvar] = 
	ri_eval_comp_var(&gcomp_var[cvar], cvar, vectlevel[level]->numeric->values, vectlevel[level]->numeric->values, TRUE);
      satisfied = ((fabsf (vectlevel[level]->numeric->values[cvar] - 1)) < MAX_APPROX);
...
Bool is_num_prec_satisfied_after_start (int cvar, int level)
{
  ...
  satisfied = ((fabsf (vectlevel[level]->numeric->values_after_start[cvar] - 1)) < MAX_APPROX);
  ...
  if (satisfied)
    ...
    {
      vectlevel[level]->numeric->values_after_start[cvar] = 
	ri_eval_comp_var(&gcomp_var[cvar], cvar, vectlevel[level]->numeric->values_after_start, 
			 vectlevel[level]->numeric->values_after_start, TRUE);
      satisfied = ((fabsf (vectlevel[level]->numeric->values_after_start[cvar] - 1)) < MAX_APPROX);
    }
...
int is_num_prec_unsatisfied_applying_action(int cvar, TimeSpec prec_time, int level, int act) 
{
  ...
  switch (prec_time) {
  case AT_START_TIME:
    memcpy(temp_values, vectlevel[level]->numeric->values, gnum_comp_var * sizeof(float));
    break;
  case AT_END_TIME:
    memcpy(temp_values, vectlevel[level]->numeric->values_after_start, gnum_comp_var * sizeof(float));
    break;
  default:
    return 1;
  }
...
//applica tutti gli effetti numerici di un'azione 
void apply_numeric_effects_set_in_common_level(int act_index, int times, int *num_set, int length)
{
  int i;
  ...
  for (i = 0; i < length; i++)
    {
      if (num_set[i] < 0)
	{
	  apply_numeric_effect_in_common_level(num_set[i], times, Hvar.common_max_values, Hvar.common_min_values, val_max, val_min);
	}
    }

  memcpy(Hvar.common_max_values, val_max, gnum_comp_var * sizeof(float));
  memcpy(Hvar.common_min_values, val_min, gnum_comp_var * sizeof(float));
  
}


//applica l'effetto numerico i-esimo
void apply_numeric_effect_in_common_level (int num_eff, int times, float *max_in, float *min_in, float *max_out, float *min_out)
{
  ...
  //versione non ricorsiva della eval_comp_var
  switch (gcomp_var_effects[num_eff].l_operator)
    {
    case INCREASE_OP:

      max_out[first_op]+=(times*max_in[second_op]);

      break;
    case DECREASE_OP:
 
      min_out[first_op]-=(times*max_in[second_op]);
      break;
    case SCALE_UP_OP:
    ...
    case SCALE_DOWN_OP:
      ...
      break;
    case ASSIGN_OP:
      if( max_in[first_op]<max_in[second_op])
	{
	
	  max_out[first_op]=max_in[second_op];
       
	}

      if( min_in[first_op]>min_in[second_op])
	{
	
	  min_out[first_op]=min_out[second_op];
  ...
  // X BETTINI io non farei aggiornamento
      ...
      for (i = 0; i < gnum_comp_var; i++)
	{
	  ...
	  Hvar.common_max_values[i] = ri_eval_comp_var(&gcomp_var[i], i, Hvar.common_max_values, Hvar.common_min_values, TRUE);
	  Hvar.common_min_values[i] = ri_eval_comp_var(&gcomp_var[i], i, Hvar.common_max_values, Hvar.common_min_values, FALSE);
...
float try_num_eff_in_level (int cv_index, float *in_vect, float *out_vect,Bool effect)
{
  ...
  switch (cv->l_operator)
    ...
    case FIX_NUMBER:
    case VARIABLE_OP:
       res = in_vect[cv_index];
       break;
    case MUL_OP:
      res = try_num_eff_in_level (first_op, in_vect, out_vect,FALSE) * try_num_eff_in_level(second_op, in_vect, out_vect,FALSE);
      break;
    case DIV_OP:
      ...
      if (tmp == 0)
	{
      ...
      res = try_num_eff_in_level (first_op, in_vect, out_vect,FALSE) / tmp;
      break;
    case PLUS_OP:
      res = try_num_eff_in_level (first_op, in_vect, out_vect,FALSE) + try_num_eff_in_level (second_op, in_vect, out_vect,FALSE);
      break;
    case MINUS_OP:
      ...
      break;
    case UMINUS_OP:
      res = -try_num_eff_in_level (first_op, in_vect, out_vect,FALSE);
      break;
    case INCREASE_OP:
      res = in_vect[first_op] + try_num_eff_in_level (second_op, in_vect, out_vect,FALSE);
      break;
    case DECREASE_OP:
      res = in_vect[first_op] - try_num_eff_in_level (second_op, in_vect, out_vect,FALSE);
      break;
    ...
    case SCALE_DOWN_OP:
      ...
      break;
    case ASSIGN_OP:
      res = try_num_eff_in_level (second_op, in_vect, out_vect,FALSE);
      break;
    default:
      printf ("\n\nnot considered\n\n");
      exit (2);
...
void set_numeric_flag (int from_ef_conn, int to_ef_conn)
{
  int i, j;
...
void copy_num_values_from_level_to_level (int level_from, int level_to, Bool check_variations)
{
  ...
  if (check_variations)
    ...
    memset (vectlevel[level_from]->numeric->w_is_used, 0, gnum_comp_var * sizeof (short));
...
void make_false_all_checks_on_not_init (void)
{
  ...
  for (i = 0; i < gnum_comp_var; i++)
    {
      cv = &gcomp_var[i];
      if ((cv->l_operator== VARIABLE_OP) && (cv->first_op == -1))
	{
	  ...
	  for (tmpil = il; tmpil; tmpil = tmpil->next)
	    {
	      switch (gcomp_var[tmpil->item].l_operator )
		{
		case EQUAL_OP:
		case GREATER_THAN_OP:
		case GREATER_OR_EQUAL_OP:
		case LESS_THAN_OP:
		case LESS_THAN_OR_EQUAL_OP:
		  gcomp_var[tmpil->item].l_operator= FIX_NUMBER;
...
void insert_values_unsup_num_fact (int status, int fact, int level)
{
  ...
  if (!fct_pos)
    fct_pos =(int *) calloc(max_num_value, sizeof(int));
  if (!fct_level)
    fct_level =(int *) calloc(max_num_value, sizeof(int));
  ...
  switch (status)
      ...
      for (i = 0; i < num_facts; i++)
	{
	  ...
	  for (l = fct_level[i]; l <= GpG.curr_plan_length; l++)
	    if (vectlevel[l]->numeric->w_is_used[f])
	      {
...
{
  int i, j;
  gis_not_appliable =(int *) calloc (gnum_ef_conn / 32 + 1, sizeof (int));
  for (i = 0; i < gnum_ef_conn; i++)
    {
      //START
      for (j = 0; j < gef_conn[i].num_PC; j++)
      ...
      if (gef_conn[i].sf)
	{
	  ...
	  for (j = 0; j < gef_conn[i].sf->num_PC_overall; j++)
	    {
	      if (gef_conn[i].sf->PC_overall[j] < 0)
		if (gcomp_var[-gef_conn[i].sf->PC_overall[j]].l_operator == FIX_NUMBER)
	  ...
	  for (j = 0; j < gef_conn[i].sf->num_PC_end; j++)
	    {
	      if (gef_conn[i].sf->PC_end[j] < 0)
		if (gcomp_var[-gef_conn[i].sf->PC_end[j]].l_operator == FIX_NUMBER)
...
ri_eval_comp_var (CompositeNumVar * cv, int index ,float *max_values,float *min_values,Bool Sign)
{
  ...
  switch (cv->l_operator)
    ...
    case EQUAL_OP:
      ...
      // MODIFICATO X UGUALIANZA
      // prima non veniva gestito e dava un messaggio di errore... Adesso controllo che gli intervalli [min, max] di first_op e second_op ammettano un'intersezione

      cv_value = ((ri_eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op,max_values,min_values,TRUE ) > 
		   ri_eval_comp_var(&(gcomp_var[cv->second_op]), cv->second_op,max_values,min_values,FALSE)) && 
		  (ri_eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op,max_values,min_values,FALSE) < 
		   ri_eval_comp_var(&(gcomp_var[cv->second_op]), cv->second_op,max_values,min_values,TRUE)));
      return cv_value;
    case INCREASE_OP:
...
Bool does_action_support_numeric_precond_in_down_level(int index,int action_pos, int level, int down_level)
{
  ...
  if (modifie==NULL)
    modifie =(int *) calloc(gnum_block_compvar, sizeof(int));
  if (num_value2 == NULL)
    num_value2= (float*) calloc(gnum_comp_var, sizeof (float));

  change = FALSE;
 
  if(down_level< get_prev(level))
    {
      ...
      for(k=0;k<gef_conn[act_num].num_numeric_effects;k++)
	{
 	 ...
 	 if ((*numeric_effect).lval == gcomp_var[index].first_op)
	   {
	     if (gcomp_var_effects[numeric_effect->index].l_operator == ASSIGN_OP)
	       return FALSE;
	     else
	       change = TRUE;
  ...
  for(j=down_level;j<level;j++)
	...
	{	   
	  ...
	  if ((*numeric_effect).is_at_start)
	    {
	      if((gcomp_var_effects[(*numeric_effect).index].l_operator==ASSIGN_OP) &&  ((*numeric_effect).lval==gcomp_var[index].first_op))
		return FALSE;
...
Bool does_action_remotion_support_numeric_precond_in_down_level(int index, int level, int down_level)
{
  ...
  if (down_level == get_prev(level))
    {
      if (GpG.durative_actions_in_domain)
	{
	  if (!is_num_prec_satisfied_after_start(index, down_level))
	    return FALSE;
	}
      else
	{
	  if (!is_num_prec_satisfied(index, down_level))
	    return FALSE;
	}
...
IntList *get_numvar_changed_at_start_by(int action)
{
  int i;
  IntList *res = NULL, *tmp = NULL;

  if (!gef_conn[action].is_numeric)
    return NULL;

  for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
    {
      if (gcomp_var[gef_conn[action].numeric_effects[i].lval].l_operator != VARIABLE_OP)
	continue;

      if (gef_conn[action].numeric_effects[i].is_at_start)
	{
	  tmp = get_IntList();
	  tmp->item = gef_conn[action].numeric_effects[i].lval;
	  tmp->next = res;
	  res = tmp;
	}
    }
...
IntList *get_numvar_changed_at_end_by(int action)
{
  int i;
  IntList *res = NULL, *tmp = NULL;
...
IntList *get_numvar_changed_by(int action)
{
  int i;
  IntList *res = NULL, *tmp = NULL;
