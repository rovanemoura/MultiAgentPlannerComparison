...
void allocate_reachability_compvar_information_data(void)
{
  ...
  if(Hvar.numeric_precs_in_relaxed_plan ==NULL)
    Hvar.numeric_precs_in_relaxed_plan = (int *)calloc(gnum_block_compvar,sizeof(int));
  ...
  Hvar.ri_tot_cost_of_numeric_facts=(float *) calloc (gnum_comp_var, sizeof (float));
  Hvar.ri_cost_of_numeric_facts=(float *)  calloc (gnum_comp_var, sizeof (float));
  Hvar.ri_duration_of_numeric_facts=(float *)  calloc (gnum_comp_var, sizeof (float));
  Hvar.ri_num_actions_of_numeric_facts=(int *) calloc (gnum_comp_var, sizeof (int));
  ...
  memset(Hvar.ri_best_increase_for_compvar, UNREACHABLE_ACTION ,  gnum_comp_var* sizeof(int));
  memset(Hvar.ri_best_decrease_for_compvar, UNREACHABLE_ACTION ,  gnum_comp_var* sizeof(int));
  memset(Hvar.ri_best_assign_for_compvar, UNREACHABLE_ACTION ,  gnum_comp_var* sizeof(int));
...
void allocate_reachability_information_data()
{
  ...
  if (!Hvar.bit_vect_relevant_facts)
    Hvar.bit_vect_relevant_facts = alloc_vect (gnum_ft_block);
  ...
  Hvar.ri_tot_cost_of_facts= (float *)  calloc (gnum_ft_conn, sizeof (float));
  Hvar.ri_cost_of_facts = (float *) calloc (gnum_ft_conn, sizeof (float));
  Hvar.ri_duration_of_facts = (float *) calloc (gnum_ft_conn, sizeof (float));
  Hvar.ri_num_actions_of_facts =(int *)  calloc (gnum_ft_conn, sizeof (int));
  ...
  memset(Hvar.ri_best_act_for_facts, UNREACHABLE_ACTION ,  gnum_ft_conn* sizeof(int));

  Hvar.ri_num_actions_of_actions=alloc_vect (gnum_ef_conn);
  Hvar.ri_cost_of_actions=  (float *)  calloc (gnum_ef_conn, sizeof (float));
  ...
  last_best_act_cost =(last_cost *) calloc(gnum_ef_conn, sizeof(last_cost));
...
void ri_apply_action_effects_to_max_min_value(int action_number, int times)
{
  ...
  memcpy(val_max, Hvar.max_values, gnum_comp_var * sizeof(float));
  memcpy(val_min, Hvar.min_values, gnum_comp_var * sizeof(float));
  ...
  for(i=0;i<gef_conn[action_number].num_numeric_effects;i++)
	...
	{
	case INCREASE_OP:
	  Hvar.max_values[numeric_effect->lval] = val_max[numeric_effect->lval]+ (times* val_max[gcomp_var_effects[numeric_effect->index].second_op]);
	  Hvar.modifieds_values=1;
	  break;

	case DECREASE_OP:
	  Hvar.min_values[numeric_effect->lval] = val_min[numeric_effect->lval] - (times* val_max[gcomp_var_effects[numeric_effect->index].second_op]);
	  Hvar.modifieds_values=1;
	  break;

	case ASSIGN_OP:
	    {
	      float max, min,tmp = Hvar.max_values[numeric_effect->lval];
	      max = MAX(MAX(tmp, val_max[numeric_effect->lval]), 
			MAX(Hvar.max_values[gcomp_var_effects[numeric_effect->index].second_op], 
			    val_max[gcomp_var_effects[numeric_effect->index].second_op]));

	      if (max != tmp)
		Hvar.modifieds_values=1;
	
	      tmp = Hvar.min_values[numeric_effect->lval];
	      min = MIN(MIN(tmp, val_min[numeric_effect->lval]), 
			MIN(Hvar.min_values[gcomp_var_effects[numeric_effect->index].second_op], 
			    val_min[gcomp_var_effects[numeric_effect->index].second_op]));
...
int cri_compute_action_cost (int pos,int times)
{
  ...
  **/  
  if (Hvar.ri_num_actions_inserted >= MAX_LENGTH_H) {
#ifdef __MY_OUTPUT__
...
int cri_get_best_action_for_fct (int fact_pos)
{
  ...
  **/
  ...
  if(GpG.cri_intermediate_levels==EXECUTION_INTERMEDIATE_REACHAB_INFORM)
    {
	if(GET_BIT ( Hvar.initial_relaxed_bit_vect_facts, fact_pos))
	    return  INITIAL_ACTION;


      if( Hvar.ri_best_act_for_facts[fact_pos] == UNREACHABLE_ACTION) {  
	  ...
	  for(i=0;i<gef_conn[Hvar.ri_best_decrease_for_compvar[index]].num_numeric_effects;i++)
	    {
	      ...
	      if(GET_BIT(Numeric.Affects_Matrix[numeric_effect->lval],gcomp_var[index].first_op))
		{
		  k= (int) fabs(ceilf((Hvar.min_values[gcomp_var[index].first_op]+0.05-Hvar.max_values[gcomp_var[index].second_op]) /Hvar.max_values[gcomp_var_effects[numeric_effect->index].second_op]));   
		  if(k==0)
		    k++;  
	  ...
	  for(i=0;i<gef_conn[Hvar.ri_best_decrease_for_compvar[index]].num_numeric_effects;i++)
	    {
	      ...
	      if(GET_BIT(Numeric.Affects_Matrix[numeric_effect->lval],gcomp_var[index].first_op))
		{
		  k= (int) fabs(ceilf((Hvar.min_values[gcomp_var[index].first_op]-Hvar.max_values[gcomp_var[index].second_op]) /Hvar.max_values[gcomp_var_effects[numeric_effect->index].second_op])); 
		  if(k==0)		    
		    k++;  
	  ...
	  for(i=0;i<gef_conn[Hvar.ri_best_increase_for_compvar[index]].num_numeric_effects;i++)
	    {
	      ...
	      if(GET_BIT(Numeric.Affects_Matrix[numeric_effect->lval],gcomp_var[index].first_op))
		{
		  k= (int) fabs(ceilf((Hvar.max_values[gcomp_var[index].first_op]+0.05-Hvar.min_values[gcomp_var[index].second_op]) /Hvar.max_values[gcomp_var_effects[numeric_effect->index].second_op]));
		  if(k==0)		    
		    k++;
	  ...
	  for(i=0;i<gef_conn[Hvar.ri_best_increase_for_compvar[index]].num_numeric_effects;i++)
	    {
	      ...
	      if(GET_BIT(Numeric.Affects_Matrix[numeric_effect->lval],gcomp_var[index].first_op))
		{
		  k= (int) fabs(ceilf((Hvar.max_values[gcomp_var[index].first_op]-Hvar.min_values[gcomp_var[index].second_op]) /Hvar.max_values[gcomp_var_effects[numeric_effect->index].second_op]));
		  if(k==0)
		    k++;
    ...
    **/
	...
	if(GpG.cri_initial_or_update==0)
	  { /* Initial CRI */
	    ...
	    num_act +=(int) Hvar.ri_num_actions_of_numeric_facts[-idx];
	  }
	else
  ...
  if (gef_conn[action].sf != NULL)
      ...
      for (i = 0; i < gef_conn[action].sf->num_PC_end; i++)
	{
	  ...
	  **/
	  ...
	  else if(best_act == UNREACHABLE_ACTION)
	    { 
	      /**
...
/**
 * Name:remove_actions_with_prec_from_bitvect
 * Purpose: Reset from bit_vect the bit values corresponding to 
 *  the indexes of the actions having fact_pos as precondition.
 * Main functions used: None 
 * Called by: cri_insert_ftcost 
 */
void remove_actions_with_prec_from_bitvect(int fact_pos, int * bit_vect)
{
  ...
  for(i=0;i< gft_conn[fact_pos].num_PC;i++)
   { 
     j=gft_conn[fact_pos].PC[i];
     if(j>=0)
       {
	 if( GET_BIT(bit_vect,j) )
	   {
	     RESET_BIT(bit_vect,j);
	     printf("\n RESET  fact %d act %d ", fact_pos, j);
	   }
       }
     else
       printf("\n Warning: unvalid index ");
			...
			int num_actions,int best_ef)
{
  ...
  if(DEBUG3)
      printf("\nCRI set fact cost: %s, %.2f num act %d best act %s ", print_ft_name_string(fact_pos, temp_name), cost, num_actions,  print_op_name_string(best_ef, temp_name));
  ...
  **/ 
  ...
  if(Hvar.ri_best_act_for_facts[fact_pos] == UNREACHABLE_ACTION || 
      ...
      Hvar.ri_tot_cost_of_facts[fact_pos]> totcost) )
    {
      ...
      if(Hvar.ri_best_act_for_facts[fact_pos] != UNREACHABLE_ACTION && GpG.improve_reachability)
	remove_actions_with_prec_from_bitvect(fact_pos, Hvar.temp_applied);
  ...
  **/
  if (GpG.first_execution_cri==0)
      ...
      if(Hvar.dg_facts_min_duration[fact_pos] > duration)
	{
	  ...
	  if(GpG.timed_facts_present && GpG.improve_reachability )
	   remove_actions_with_prec_from_bitvect(fact_pos, Hvar.temp_applied); 
	}
...
void cri_insert_numeric_ftcost (int fact_pos, int action, int num_eff, int op_index)
{
	...
	if (is_fact_in_delete_effects (index, gef_conn[index].sf->A_start[i]))
	  {
	    if (gef_conn[index].sf->A_start[i] >= 0)
	      cri_insert_ftcost (gef_conn[index].sf->A_start[i], MAX_COST, 
			     duration- act_dur, MAX_INT_COST, index);
...
void cri_heuristic_for_action_numeric_reach (int action, int level)
{
  static int *initial_local_bit_vect_facts=NULL;
  register int i, k, x;
  float max_prec_cost, max_prec_duration, new_cost, new_duration;
  ...
  **/
  ...
  if(num_prec_vector==NULL)
    ...
    memset (num_prec_vector, UNREACHABLE_ACTION, gnum_ef_conn * sizeof (int));
      ...
      do
	{
	  ...
	  for (k = 0; k < gef_conn[i].num_A; k++)
		...
		for(aus_counter=0;aus_counter< gft_conn[gef_conn[i].A[k]].num_PC;aus_counter++)
		  {
		    if(num_prec_vector[gft_conn[gef_conn[i].A[k]].PC[aus_counter]] == UNREACHABLE_ACTION)
	  ...
	  if (gef_conn[i].sf)
	      ...
	      if (gef_conn[i].sf->A_start[k] >= 0)
		{
		  ...
		  if (GET_BIT (Hvar.local_bit_vect_facts, gef_conn[i].A[k]) == 0)
			...
			*/
			if(num_prec_vector[gft_conn[gef_conn[i].sf->A_start[k]].PC[aus_counter]] == UNREACHABLE_ACTION)
...
Bool ri_set_best_for_compvar(int action, int * True_num,float *vmax, float *vmin,int *num_prec_vector)
{
	      ...
	      while (temp)
		{
		  ...
		  if (temp & FIRST_1)
			...
			{	  
			  ...
			  if(gcomp_var[y].l_operator==LESS_THAN_OR_EQUAL_OP||gcomp_var[y].l_operator==LESS_THAN_OP)		      
			    continue;
			  ...
			  if(Hvar.ri_best_increase_for_compvar[y]<0)
			      ...
			      if(!GET_BIT(True_num,y))
				{
					...
					while (temp2)
					  {
						...
						if(num_prec_vector[x] == UNREACHABLE_ACTION)
						  num_prec_vector[x] = 
			      ...
			      if(gcomp_var[y].l_operator==EQUAL_OP)
				{
	      ...
	      while (temp)
		{
		  ...
		  if (temp & FIRST_1)
		      ...
		      if(GET_BIT(Numeric.Affects_Matrix[numeric_effect->lval],y))
			{
			  ...
			  if(gcomp_var[y].l_operator==GREATER_OR_EQUAL_OP||gcomp_var[y].l_operator==GREATER_THAN_OP)		      
			    continue;
  ...
  for(j=0;j<gnum_comp_var;j++)
    if(gcomp_var[j].l_operator==VARIABLE_OP)
  ...
  for ( i = 0; i < gnum_comp_var; i++)
    {
      if(!GET_BIT(Numeric.is_a_Pc,i))
	continue;
      loc_dg_num_facts_array[i]  = update_num_dg_fact_list(i, &level_init, Hvar.ri_num_actions_of_numeric_facts[i], Hvar.ri_best_increase_for_compvar[i],Hvar.ri_best_decrease_for_compvar[i], Hvar.ri_best_assign_for_compvar[i],Hvar.ri_duration_of_numeric_facts[i] ,Hvar.ri_cost_of_numeric_facts[i] );
...
void compute_reachability(State * initial_state)
{
   ...
   * Fase iniziale
   */  
  Hvar.temp_applied= applied = alloc_vect (gnum_ef_block);
  To_be_Executed = alloc_vect (gnum_ef_block);  
 ...
 //F e' lo stato,un bit-vector che asserisce quali fatti sono veri, viene aggiornato ad ogni passaggio
  ...
  if(Numeric.ri_prec_vector==NULL)
 ...
 do
      ...
      for(j=0;j<gnum_comp_var;j++)
	{
	  ...
	  if(GET_BIT(relevant_vars,j))
		 ...
		 * o entrambi
		 */
		if (apply_best_assign_for_var(j,vmax,vmin))
		  {
		    values_changed = TRUE;
		  }
		if(num_Pc_relevant_pos[j]>0)
		  {
		    if (apply_best_increase_for_var(j,vmax))
		      values_changed = TRUE;
		  }
		if(num_Pc_relevant_neg[j]>0)
		  {
		    if (apply_best_decrease_for_var(j,vmax,vmin))
		      values_changed = TRUE;
		  }
  ...
  */
   for (i = 0; i < gnum_ef_conn; i++)
       if (!GET_BIT (applied, i))
	 {
	   ...
	   gef_conn[i].level = -1;
#ifdef __TEST__
	   printf("\naction %d -- %s -- ha level -1\n",i, print_op_name_string(i, temp_name));
#endif
	 }
     /*
...
void cri_set_init_numeric_fact(int fact_pos)
{
  ...
  Hvar.ri_num_actions_of_numeric_facts[fact_pos]=0;
