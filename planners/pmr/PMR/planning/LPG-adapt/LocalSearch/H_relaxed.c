 ...
 **
 * I check if there is some temporal intervals that could 
 * be associated to action act_ops and which are compatible with time t
 **/
float search_for_pc_intervals(float t, int act_pos, int lev, int *num_Timed_Prec) {
  ...
  *num_Timed_Prec = 0;
  ...
  // New temporary vector for memory assignments
...
Bool update_dg_fact_list(int fact_pos, int *level_ptr, int num_actions, int best_act, float duration, float cost, int related_fact)
{
  ...
  */
  ...
  if(best_act<0 && best_act!=INITIAL_ACTION)
    {
      dg_fact_node->num_actions=MAX_INT_COST;
...
int number_of_actions_for_unsup_numeric_precondition(int fact_pos, float A_to, float B_from)
{

  int k=1;
  float diff=0.0;
#ifdef __TEST_ASSIGN__
  int k1=0,assign=0;
#endif
...
s
   ...
   *****/
   ...
   switch (gcomp_var[fact_pos].l_operator)
    {
	 ...
	 if( Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op]!=MIN_NEG_VALUE && (Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op]< A_to ))
	   return 1;      
	 ...
	 if( Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op]!=MIN_NEG_VALUE && ((B_from - Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op] ) > Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]))
	{
	  k=1;
	  diff= fabs( A_to - Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op] ); 
	  ...
	  k1= (int) fabs(ceilf((  A_to + MAX_APPROX - B_from )/Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]));

#endif

	 k+= (int) fabs(ceilf((diff+ MAX_APPROX)/Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]));
	 if(k<=0)
	   k=1;  
#ifdef __TEST_ASSIGN__
	 if(k1!=k)
	   { 
	     if(assign)
	       printf("\n EVAL: LESS_THAN_OP ASSIGN: k1=%d -- ",k1);
	     else
	       printf("\n EVAL: LESS_THAN_OP INCREASE: k1=%d -- ",k1);

	     printf(" k=%d -- A_to = %.3f -- B_from=%.3f --Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]=%.3f -- Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op] =%.3f -- diff=%.3f ",k, A_to, B_from , Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op], Hvar.ri_min_assign_for_compvar[gcomp_var[fact_pos].first_op],diff);
	   }
#endif
...
#ifdef __TEST_ASSIGN__
	  k1= (int) fabs(ceilf((A_to- B_from)/Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]));


#endif 
	k+= (int) fabs(ceilf(diff/Hvar.ri_max_decrease_for_compvar[gcomp_var[fact_pos].first_op]));
	if(k<=0)
	  k=1;  
	...
	if( k1!=k)
	  {
	    if(assign)
	      printf("\n EVAL: LESS_THAN_OR_EQUAL_OP ASSIGN:  k1=%d -- ",k1);
	    else
	      printf("\n EVAL: LESS_THAN_OR_EQUAL_OP INCREASE:  k1=%d -- ",k1);
	...
	if( Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]!=MIN_NEG_VALUE && (Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]> A_to ))
	  return 1;

	if(Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op] <=0)
	  return  MAX_INT_COST;

	if( Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]!=MIN_NEG_VALUE && ( Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]-B_from ) > Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op])
	{
	  k=1;
	  diff= ( A_to  - Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]); 
	 ...
	 k1= (int) fabs(ceilf(( A_to +MAX_APPROX- B_from)/Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]));

#endif 
	k+= (int) fabs(ceilf((diff+MAX_APPROX)/Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]));
	if(k<=0)
	  k=1;  
	...
	if(k1!=k)
	  {
	    if(assign)
	      printf("\n EVAL: GREATER_THAN_OP ASSIGN:  k1=%d -- ",k1);
	    else
	      printf("\n EVAL: GREATER_THAN_OP INCREASE: k1=%d -- ",k1);


	    printf(" k=%d -- A_to = %.3f -- B_from =%.3f -- Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]=%.3f --  Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]=%.3f -- diff=%.3f ",k, A_to , B_from, Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op],  Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op],diff);
	  }
#endif
...
#ifdef __TEST_ASSIGN__
	 k1= (int) fabs(ceilf((A_to -B_from)/Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]));

#endif 


	k+= (int) fabs(ceilf(diff/Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]));
	if(k<=0)
	  k=1;  
	...
	if(k1!=k)
	  {
	    if(assign)
	      printf("\n EVAL: GREATER_OR_EQUAL_OP ASSIGN:  k1=%d -- ",k1);
	    else
	      printf("\n EVAL: GREATER_OR_EQUAL_OP INCREASE: k1=%d -- ",k1);
	    printf(" k=%d -- A_to = %.3f --  B_from =%.3f -- Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op]=%.3f --  Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op]=%.3f -- diff=%.3f ",k, A_to,  B_from , Hvar.ri_max_increase_for_compvar[gcomp_var[fact_pos].first_op],  Hvar.ri_max_assign_for_compvar[gcomp_var[fact_pos].first_op],diff);
...
int num_action_for_unsup_num_precondition(int fact_pos, int level)
{
  ...
  B_from = ri_eval_comp_var(&gcomp_var[gcomp_var[fact_pos].first_op], gcomp_var[fact_pos].first_op,
			    vectlevel[level]->numeric->values, vectlevel[level]->numeric->values, TRUE);
  A_to = ri_eval_comp_var(&gcomp_var[gcomp_var[fact_pos].second_op], gcomp_var[fact_pos].second_op,
			  vectlevel[level]->numeric->values, vectlevel[level]->numeric->values, TRUE);
  
  return  number_of_actions_for_unsup_numeric_precondition(fact_pos, A_to, B_from);
  
}


int num_actions_unsup_num_precondition_in_common_level(int fact_pos, int level)
{
  float A_to, B_from;
   ...
   switch (gcomp_var[fact_pos].l_operator)
    {
       ...
       case LESS_THAN_OR_EQUAL_OP:
	 B_from = ri_eval_comp_var(&gcomp_var[gcomp_var[fact_pos].first_op], gcomp_var[fact_pos].first_op,
				   Hvar.common_max_values, Hvar.common_min_values, FALSE);
	 A_to = ri_eval_comp_var(&gcomp_var[gcomp_var[fact_pos].second_op], gcomp_var[fact_pos].second_op,
				   Hvar.common_max_values, Hvar.common_min_values, TRUE);
...
compute_constr_fact (int constraint, int fact_pos, int level, int initialize, int orig_level, float *cost, float *end_time)
{
  ...
  for (i = 0; i < gef_conn[action].num_PC; i++)
      ...
      if (fact_is_supported (el, lev1)|| GET_BIT (Hvar.bit_vect_facts, el) )
	{
      ...
      num_constr += (int) compute_constr_fact (constraint, el, lev1, 0, orig_level, cost, end_time);
...
insert_action_inlist (int pos, int fact)
{
  ...
  /* ADAPT */
  if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
    {
      int o_pos= pos;
      if(gef_conn[o_pos].orig_or_position>0)
	o_pos=gef_conn[o_pos].orig_or_position;
 
      if(Hvar.input_plan_actions[o_pos]<=0)
	Hvar.input_plan_cost++;
      else
	if(GpG.adapt_all_diff)
	  Hvar.input_plan_cost--;
	      
      Hvar.input_plan_actions[o_pos]--;
    }
 /* ADAPT */
	  ...
	  for (i = 0, temp = 0.0; i < gef_conn[act_pos].num_PC; i++)
	      ...
	      if (el < 0)
		{
		  if (!is_num_prec_satisfied  (el,level))
			...
			if(num_actions<=0)
			  printf("\n Warning num_actions [L] = %d ", num_actions);
  ...
  if(max_time_for_timed_fact<MAXFLOAT && GpG.timed_facts_present)
    {
      ...
      for (i=0; i < gnum_timed_facts; i++)
	{
	  for (j = 0; j < gnum_tmd_interval_block[i]; j++)
	    {
	      bit_temp = gtimed_fct_bitarray[i][j];
	      ...
	      while (bit_temp)
		{
		  ...
		  if (bit_temp & FIRST_1)
		    {
		      if (n_cost->act_time + max_time_for_timed_fact > gtimed_fct_vect[i][j*32+k].slack) 
			{
			  total ++;
			  if (DEBUG3) 
			    {
			      printf("\nTF-BA: Increase cost for BA %s: time(BA) %.2f, time of previous actions in Rplan %.2f, slack %.2f", print_op_name_string(act_pos,temp_name),n_cost->act_time,max_time_for_timed_fact,gtimed_fct_vect[i][j*32+k].slack);
			      printf("for timed fact %s", print_ft_name_string(gtimed_fct_vect[i][j*32+k].position,temp_name));
			    }
			}
		    }
		  bit_temp <<= 1;
...
relaxed_neighborhood_evaluation (int act_pos, int level,  node_cost_list n_cost, float max_time_for_timed_fact)
{
 ...
 n_cost->num_actions+=Hvar.ri_num_actions_define_cost;
  ...
  n_cost->act_time=0.0; // Va calcolato bene per ora evito di consisderarlo
...
float numeric_goal_interference(int action, int level)
{

  int i, var, toCheck;
  float cost;
  static int *modified = NULL;
  static float *values_after = NULL;
  IntList *tmp;
  DescNumEff *numeff;

  cost = 0.0;

  if (values_after == NULL)
    values_after = (float *)calloc(gnum_comp_var, sizeof(float));
  if (modified == NULL)
    modified = alloc_vect(gnum_block_compvar);
  
  memcpy(values_after, vectlevel[level]->numeric->values, gnum_comp_var * sizeof(float));
  memset(modified, 0, gnum_block_compvar * sizeof(int));
  
  /*
    for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
    if (gef_conn[action].numeric_effects[i].is_at_start) 
    apply_numeric_effect_to(action, i, values, values_after, modified);
    
    refresh(values_after, modified, -1);
    
    for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
    if (!gef_conn[action].numeric_effects[i].is_at_start) 
    apply_numeric_effect_to(action, i, values_after, values_after, modified);
    
    refresh(values_after, modified, -1);
  */
  
  for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
    apply_numeric_effect_to(action, i,  vectlevel[level]->numeric->values, values_after, modified);
  
  for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
    {
      numeff = &gef_conn[action].numeric_effects[i];
      var = numeff->lval ;

      //per ognuna delle compvar che sono modificate da questo effetto numerico
      for (tmp = gcomp_var[var].affects; tmp; tmp = tmp->next)
	{
	  ...
	  if (vectlevel[level]->numeric->w_is_goal[toCheck] > 0)
	      ...
	      if (!(fabsf (ri_eval_comp_var(&gcomp_var[toCheck], toCheck, values_after, values_after, TRUE) - 1)) < MAX_APPROX)
		{
		  ...
		  if (DEBUG5)
		    {
		      printf("\n>>> NUMERIC THREAT: %s", print_ft_name_string(-toCheck, temp_name));
		      print_cvar_tree(toCheck, level);
		      printf("\n>>>          %s", print_numeric_ft_string(numeff->index, temp_name, gcomp_var_effects));
		    }
		  
		  if (GpG.numeric_threats_mode == NUMERIC_THREATS_MAX)
		    {
		      cost = MAX(cost, 
				 number_of_actions_for_unsup_numeric_precondition(toCheck,values_after[gcomp_var[toCheck].second_op], 
										  values_after[gcomp_var[toCheck].first_op]));
		    }
		  else if (GpG.numeric_threats_mode == NUMERIC_THREATS_TOT)
		    {
		      cost += number_of_actions_for_unsup_numeric_precondition(toCheck, values_after[gcomp_var[toCheck].second_op], 
									       values_after[gcomp_var[toCheck].first_op]);
		    }
		  else //NUMERIC_THREATS_NUM
		    cost++;
		}
	    } 
	}
  ...
  if (DEBUG5)
    {
      ...
      if (cost > 0.0)
	printf("\nNUMERIC: w_is_goal evaluation: increasing cost of %s --> add %.2f\n", 
	       print_op_name_string(action, temp_name), cost);
    }
                           ...
                           float max_time_for_timed_fact, node_cost_list max_n_cost)
{
  ...
  //XXXXXXXXXXXXXXXX
  if (Hvar.action!=NULL && (Hvar.action->constraint_type == C_T_REMOVE_ACTION) && (act_pos == Hvar.action->act_pos))
    {
      n_cost->weight= MAX_COST;
      n_cost->act_cost= MAX_COST;
      n_cost->act_time= MAX_COST;
	      ...
	      if (el < 0)
		{
		  if (!is_num_prec_satisfied_in_common_level (el))
		    {
		      ...
		      {
			//get_dg_num_fact_cost (-el, level - 1, &loc_dg_num_cost); 
			num_actions = num_actions_unsup_num_precondition_in_common_level(-el, level);

			if(num_actions<=0)
			  printf("\n Warning num_actions [C] = %d ", num_actions);
		  ...
		  if (GpG.penalize_inconsistence_in_relaxed_plan)
		    { 
		      ...
		      if (Hvar.constr->constraint_type == C_T_UNSUP_NUM_FACT
			  /* && (Hvar.constr->fact != -el) */
			  && does_numeric_prec_support_inconsistence_in_common_level(-el))
			{
			  n_cost->numeric_fault = TRUE;
  ...
  total += numeric_goal_interference(act_pos, level);
  ...
  if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
	...
	o_pos=gef_conn[o_pos].orig_or_position;
      if(Hvar.input_plan_actions[o_pos]<=0)
	penalization+=GpG.best_act_penalization_coeff;
    }
  /* ADAPT */
	  ...
	  for(next_level=level+1;next_level<=GpG.curr_plan_length;next_level++)
	      ...
	      if(next_level==GpG.curr_plan_length)
		{
		  ...
		  for(i=0;i<gnum_comp_var;i++)
		    {
		      if(vectlevel[next_level]->numeric->w_is_used[i])
			{
			  ...
			  if(vectlevel[next_level]->numeric->values[i]<0.5)
			    {
			      ...
			      if(gcomp_var[i].l_operator==GREATER_THAN_OP || 
				 gcomp_var[i].l_operator== GREATER_OR_EQUAL_OP)
				if(vectlevel[next_level]->numeric->values[gcomp_var[i].first_op] >
			      ...
			      if(gcomp_var[i].l_operator==LESS_THAN_OP || 
				 gcomp_var[i].l_operator== LESS_THAN_OR_EQUAL_OP)
				if(vectlevel[next_level]->numeric->values[gcomp_var[i].first_op] <
				   Hvar.temp_num_level[gcomp_var[i].first_op])
				  {
	      ...
	      for(counter=0;counter<gef_conn[action_pos].num_PC;counter++)
		{
		  if(gef_conn[action_pos].PC[counter]<0)
		      ...
		      if(vectlevel[next_level]->numeric->values[-(gef_conn[action_pos].PC[counter])]<0.5)
			{
			  ...
			  if(gcomp_var[-(gef_conn[action_pos].PC[counter])].l_operator==GREATER_THAN_OP || 
			     gcomp_var[-(gef_conn[action_pos].PC[counter])].l_operator== GREATER_OR_EQUAL_OP)
			  ...
			  if(gcomp_var[-(gef_conn[action_pos].PC[counter])].l_operator==LESS_THAN_OP || 
			     gcomp_var[-(gef_conn[action_pos].PC[counter])].l_operator== LESS_THAN_OR_EQUAL_OP)
			    if(vectlevel[next_level]->numeric->values[gcomp_var[-(gef_conn[action_pos].PC[counter])].first_op] <
			       Hvar.temp_num_level[gcomp_var[-(gef_conn[action_pos].PC[counter])].first_op])
			      {
	      ...
	      for(counter=0;counter<gef_conn[action_pos].num_numeric_effects;counter++)
		{
		  if(!GET_BIT(Hvar.to_control,gef_conn[action_pos].numeric_effects[counter].lval))
		    continue;
		  if(gcomp_var[gef_conn[action_pos].numeric_effects[counter].index].l_operator==ASSIGN_OP)
		    {
		      num_vars--;
...
dg_action_cost (neighb_list neighb_act)
{
  ...
  int level, i, j = 0, k1, diff = 0, next_level=0, init_na, ord;
  register EfConn *act;
  ...
  float init_cost, init_w;
  float time_Timed_Prec=-1.0;
  float future_timed_cost;
  int num_Timed_Prec=0;
 ...
 /* ADAPT */
  if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
    memcpy(Hvar.input_plan_actions, GpG.input_plan_actions,  gnum_op_conn * sizeof (int));
  if(GpG.adapt_all_diff==0)
    Hvar.input_plan_cost=0.0;
  else
    Hvar.input_plan_cost=GpG.num_new_actions;
  ...
  /* Counts the number of ME actions with the current one */
      ...
      if (GpG.accurate_cost >= COMPUTE_DG_LIST_COST)
	{
	  ...
	  if (gef_conn[act->position].sf)
	    {
	      apply_numeric_effects_set_in_common_level(act->position, 1, gef_conn[act->position].sf->A_start,
							gef_conn[act->position].sf->num_A_start);
	  ...
	  apply_numeric_effects_set_in_common_level(act->position, 1, gef_conn[act->position].A,
							gef_conn[act->position].num_A);
 ...
 /* Inserisco in Hvar.supported_preconds le precondizioni non supportate */
  ...
  if (FALSE)
  for (i = 0, j = 0; j < gnum_comp_var; i++, j += 32)
    {
      
      temp1 = Hvar.numeric_precs_in_relaxed_plan[i];
	  ...
	  if (temp1 & FIRST_1) 
	    {
	      ...
	      RESET_BIT(Hvar.numeric_precs_in_relaxed_plan, k1);
	      
	      if (DEBUG5) 
		{
		  printf("\n\nNUMERIC: RIEVAL NUM PREC IN RELAXED PLAN");
		  print_ft_name(-k1);
		}
	      
	      if (!is_num_prec_satisfied_in_common_level(k1))
		{
		  loc_n_cost.weight = 0.0;
		  loc_n_cost.act_cost = 0.0;
		  loc_n_cost.act_time = 0.0;
		  ...
		  temp = compute_relaxed_fact_cost (-k1, level, &loc_n_cost, level, get_action_time (act->position, level));
  ...
  if (unsat_facts > 1)
   ...
   if (GpG.accurate_cost >= COMPUTE_DG_LIST_COST)
      ...
      else
	{
      ...
      neighb_act->cost.input_plan_cost= Hvar.input_plan_cost;
      ...
      if(GpG.timed_facts_present )
	{
	      ...
	      for (j = 0; j < gnum_tmd_interval_block[i]; j++)
		{
		  ...
		  while (bit_temp)
		      ...
		      if (bit_temp & FIRST_1)
			{
			  ...
			  if (neighb_act->cost.act_time > gtimed_fct_vect[i][j*32+k].slack) 
			    {
			      neighb_act->cost.timed_fa += gtimed_fct_vect[i][j*32+k].num_act_PC;
			      if(DEBUG3)
				printf("\nTF-NeighbAct: Increase timed cost for %d new unsupported timed preconditions", gtimed_fct_vect[i][j*32+k].num_act_PC);//, neighb_act->cost.weight);
			    }
			}
	  ...
	  if (FALSE && vectlevel[*Hvar.constr->level]->action.position >= 0)
		...
		printf("\nTF-NeighbAct: Increase timed cost for %f precs that cannot be supported", future_timed_cost);//, neighb_act->cost.weight);
  ...
  if(neighb_act->cost.weight>=MAX_COST)
    {
     neighb_act->cost.weight = MAX_COST;
      ...
      neighb_act->cost.num_actions = MAX_INT_COST;
...
float compute_relaxed_fact_cost (int Fact_position, int level, node_cost_list n_cost, int action_level, float max_time_for_timed_fact)
{
  ...
  **/
  ...
  if (Fact_position >= 0) {
	...
	best_n_cost.numeric_fault = FALSE;
	...
	create_gdp_path_for(Fact_position, level, n_cost->action, &Hvar.tmp_path);
		...
		for (j = 0; (j < num_facts) && local_search.ls_continue; j++)
		  {
		    ...
		    loc_n_cost.act_cost = 0.0;
		    loc_n_cost.act_time = 0.0;
		    loc_n_cost.numeric_fault = FALSE;
  ...
  else
    {
      ...
      times=choose_best_action_for_unsup_num_fact (Fact_position, level, 
						   &best_action, &best_level, &best_n_cost,
  ...
  /* ADAPT cost */
	if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
	  if(times>0)
	  {
	    int o_pos= best_action;
	    if(gef_conn[o_pos].orig_or_position>0)
	     o_pos=gef_conn[o_pos].orig_or_position;

	    if( Hvar.input_plan_actions[o_pos]< times)
	      {
		if( GpG.adapt_all_diff )
		  Hvar.input_plan_cost-=Hvar.input_plan_actions[o_pos];

		  Hvar.input_plan_cost+=(times-1-Hvar.input_plan_actions[o_pos] );
	
	      }
	    else
	      if( GpG.adapt_all_diff )
		  Hvar.input_plan_cost-=(times-1);

	      /* Hvar.input_plan_actions is decremented of 1 unit also in  insert_action_inlist */
		 
	      Hvar.input_plan_actions[o_pos]-=(times-1);
  ...
  if (best_n_cost.numeric_fault)
    {
      if (DEBUG5)
	{
	  printf("\n\nRELAXED PLAN NUMERIC FAULT");
	}
  ...
  //XXXXXXXXXXXXXXXX
  if (Hvar.action!=NULL && (Hvar.action->constraint_type == C_T_REMOVE_ACTION) && (best_action == Hvar.action->act_pos))
    {
       Hvar.weight_facts_define_cost = MAX_COST;
       n_cost->weight = MAX_COST;
       n_cost->act_cost = MAX_COST;
  ...
  **/
  ...
  total += numeric_goal_interference(best_action, best_level);
...
// Precondizioni at start
  ...
  for (j = 0, k = 0, prec = 0.0;
      ...
      if ((el >= 0 && gft_conn[el].level>=0)  ||  CHECK_FACT_POS (el, best_level))
	{
	  ...
	  */
	  ...
	  temp =(int) compute_relaxed_fact_cost (el, level, &loc_n_cost, action_level, max_time_for_timed_fact+ get_action_time (best_action, best_level));
	  ...
	  if (CHECK_FACT_POS (el, best_level))
		...
		Azzero loc_n_cost
	      */
	      ...
	      temp = (int) compute_relaxed_fact_cost (el, level, &loc_n_cost, action_level, max_time_for_timed_fact);
  ...
  */ 
  ...
  if (Fact_position < 0) 
    {
      num_act = num_actions_unsup_num_precondition_in_common_level(-Fact_position, level);
      ...
      if (num_act == MAX_COST)
	{
      ...
      Hvar.num_actions_define_cost += num_act - 1;
    }
  //XXXXXXXXXXXXXXXXXXXXXXXXXXXX
  ...
  if (GpG.accurate_cost >= COMPUTE_DG_LIST_COST)
      ...
      if (gef_conn[best_action].sf)
	{
	  ...
	  for (num_act = num_act; num_act > 0; num_act--)
	    apply_numeric_effects_set_in_common_level(best_action, 1, gef_conn[best_action].sf->A_start,
						      gef_conn[best_action].sf->num_A_start);
      ...
      for (num_act = num_act; num_act > 0; num_act--)
	    apply_numeric_effects_set_in_common_level(best_action, 1, gef_conn[best_action].A,
						      gef_conn[best_action].num_A);
  ...
  /*
  if (Fact_position < 0 && !is_num_prec_satisfied_in_common_level(Fact_position)) 
    {
      if (DEBUG5)
	{
	  printf("\n\n>>>  BEST_ACTION DOESN'T SOLVE NUMERIC PREC IN COMMON LEVEL --> Need other actions");
	}
      
      total += compute_relaxed_fact_cost(Fact_position, level, &loc_n_cost, action_level, max_time_for_timed_fact);
    }
  */
  if (Fact_position < 0)
    SET_BIT(Hvar.numeric_precs_in_relaxed_plan, -Fact_position);
...
int relaxed_set_best_for_compvar(int i,int y)
{
  ...
  for(j=0;j<gef_conn[i].num_numeric_effects;j++)
	...
	continue;
      ...
      switch (gcomp_var_effects[numeric_effect->index].l_operator)
	{
...
choose_best_action_for_unsup_num_fact (int Fact_position, int level,
				       int *best_action, int *best_level, node_cost_list best_n_cost,
  ...
  if (gef_conn[act->position].sf)
      ...
      for (j = 0; j < gef_conn[act->position].sf->num_A_start; j++)
	{
	  ...
	  if (el < 0)
	    {
	      // Applico  eff in common level 
	      // Da sostituire con applicazione effetti normali...
	      //apply_numeric_effect_in_common_level (el,1);
	      continue;	// LAZZA
	    }
...
build_fast_relaxed_plan_for_fact ( int fact_pos, int level, int orig_level )
{
  ...
  if(fact_pos<0)
    return 0; //Estendere per domini numerici
...
Bool does_numeric_prec_support_inconsistence_in_common_level(int prec) 
{
  int incons;
  float value = 0.0;
  ...
  if (DEBUG5)
    {
      printf("\nNUMERIC CHECK FOR RELAXED PLAN PRECONDITION (COMMON LEVEL):");
      printf("\n%s", print_ft_name_string(-prec, temp_name));
    }

  if (gcomp_var[gcomp_var[prec].first_op].l_operator != VARIABLE_OP)
    return FALSE;

  if (Hvar.constr->constraint_type != C_T_UNSUP_NUM_FACT)
    return FALSE;
  
  if (is_num_prec_satisfied_in_common_level(prec))
    return FALSE;

  incons = Hvar.constr->fact;
  ...
  memcpy(val_max, Hvar.common_max_values, gnum_comp_var * sizeof(float));
  memcpy(val_min, Hvar.common_min_values, gnum_comp_var * sizeof(float));
  

  switch (gcomp_var[prec].l_operator)
    {
    case GREATER_THAN_OP:
    case GREATER_OR_EQUAL_OP:
      
      val_max[gcomp_var[prec].first_op] = val_min[gcomp_var[prec].second_op];
      value = ri_eval_comp_var (&gcomp_var[incons], incons, val_max, val_min, TRUE);
      /*
      Hvar.common_max_values[gcomp_var[prec].first_op] = Hvar.common_max_values[gcomp_var[prec].second_op];
      value = ri_eval_comp_var (&gcomp_var[incons], incons, Hvar.common_max_values, Hvar.common_min_values, TRUE);
      */
      break;
    case LESS_THAN_OP:
    case LESS_THAN_OR_EQUAL_OP:
      val_min[gcomp_var[prec].first_op] = val_max[gcomp_var[prec].second_op];
      value = ri_eval_comp_var (&gcomp_var[incons], incons, val_max, val_min, FALSE);
      /*
      Hvar.common_min_values[gcomp_var[prec].first_op] = Hvar.common_min_values[gcomp_var[prec].second_op];
      value = ri_eval_comp_var (&gcomp_var[incons], incons, Hvar.common_max_values, Hvar.common_min_values, FALSE);
      */
      break;
    default:
      break;
  ...
  if (DEBUG5)
    {
      if ((fabsf (value - 1)) < MAX_APPROX)
	{
	  printf("\nNUMERIC: PRECONDITION FOR RELAXED PLAN SOLVE INCONS");
	}
    }
  
  return ((fabsf (value - 1)) < MAX_APPROX);
  
}

/*
Bool does_numeric_prec_support_inconsistence(int prec) 
{
  int incons;
  float value = 0.0;
  ...
  if (DEBUG5)
    {
      printf("\nNUMERIC CHECK FOR RELAXED PLAN PRECONDITION:");
      printf("\n%s", print_ft_name_string(-prec, temp_name));
    }
  ...
  memcpy(Hvar.temp_values, vectlevel[*Hvar.constr->level]->numeric->values, gnum_comp_var * sizeof(float));

  incons = Hvar.constr->fact;
  
  if (incons < 0)
    incons = -incons;
  Hvar.temp_values[gcomp_var[prec].first_op] = Hvar.temp_values[gcomp_var[prec].second_op];

  
  value = ri_eval_comp_var (&gcomp_var[incons], incons, Hvar.temp_values, Hvar.temp_values, TRUE);

  if (DEBUG5)
    {
      if ((fabsf (value - 1)) < MAX_APPROX)
	{
	  printf("\nNUMERIC: PRECONDITION FOR RELAXED PLAN SOLVE INCONS --> MAX_COST");
	}
    }
