...
#include <algorithm>
#include <iostream>
#define S_ENTRY_OVERHEAD 30
#define S_LOAD_FACTOR 2
...
State::State(const State &to_copy) :
    vars(to_copy.vars), reached_lms(to_copy.reached_lms), reached_lms_cost(to_copy.reached_lms_cost) {}
*/

State::State(const State &predecessor, const Operator &op) :
	vars(predecessor.vars), reached_lms(predecessor.reached_lms), reached_lms_cost(predecessor.reached_lms_cost) {
	assert(!op.is_axiom());
...
void State::add_h_value(int h_value) {
    heuristic_values.push_back(h_value);
}

void State::get_h_values(vector<int> &h_vec) const {
	h_vec.insert(h_vec.end(), heuristic_values.begin(), heuristic_values.end());   
}
	
void State::set_pref_ops(const vector<const Operator *> &prefs) {
    pref_operators.insert(pref_operators.end(), prefs.begin(), prefs.end());
}
	
void State::get_preferred_ops(vector<const Operator *> &prefs) const {
    prefs.insert(prefs.end(), pref_operators.begin(), pref_operators.end());
}
...
size_t State::approx_num_bytes() const {
    size_t size = sizeof(State) /* State Size */ 
                ...
                + S_LOAD_FACTOR*reached_lms.size()*(S_ENTRY_OVERHEAD + sizeof(LandmarkNode *) + sizeof(hash_pointer))
                /* h_values overhead */
                + heuristic_values.capacity()*sizeof(int)
                /* pref_operators size */
                + pref_operators.capacity()*sizeof(const Operator *);
    return size;
}
