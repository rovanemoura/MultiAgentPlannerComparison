...
protected:
    ...
    int heuristic_type;
public:
	enum {REGULAR = 0, PLAN_LENGTH = 1, PLAN_COST = 2}; 
    
    int get_lower_bound(const State &state);
    void set_additional_goals(const std::vector<std::pair<int, int> >& goals);
    virtual void set_recompute_heuristic() {heuristic_recomputation_needed = true;}
    ...
    FFHeuristic(int h_type=REGULAR, bool use_cache=false);
    ~FFHeuristic();
    virtual string get_heuristic_name(){return "FF";}
