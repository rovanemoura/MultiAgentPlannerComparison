...
void write_op_name( int index ,FILE *opf)

{
  ...
  if ( !a->norm_operator &&
    ...
    fprintf(opf,"(%s", a->name );
    for ( i = 0; i < a->num_name_vars; i++ ) {
      fprintf(opf," %s", gconstants[a->name_inst_table[i]]);
    }
  }
fprintf(opf,")\n");
}
...
*/
  ...
  FILE *ofp;
  ofp = fopen(gcmd_line.plan_file_name,"w");
  for ( i = 0; i < gnum_plan_ops; i++ ) {
/*     printf("\n\nnstate:"); */
