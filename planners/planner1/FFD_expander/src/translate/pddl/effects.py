...
class Effect(object):
    ...
    def instantiate_mini(self, var_mapping, result):
        if self.parameters:
            var_mapping = var_mapping.copy() # Will modify this.
            object_lists = [objects_by_type.get(par.type_name, [])
                            for par in self.parameters]
            for object_tuple in cartesian_product(*object_lists):
                for (par, obj) in zip(self.parameters, object_tuple):
                    var_mapping[par.name] = obj
                self._instantiate_mini(var_mapping, result)
        else:
            self._instantiate_mini(var_mapping, result)
    def _instantiate_mini(self, var_mapping, result):
        condition = []
        try:
            self.condition.instantiate_mini(var_mapping,  condition)
        except conditions.Impossible:
            return
        effects = []
        self.literal.instantiate_mini(var_mapping, effects)
        assert len(effects) <= 1
        if effects:
            result.append((condition, effects[0]))
