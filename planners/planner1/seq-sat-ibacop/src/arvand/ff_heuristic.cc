...
FFHeuristic::FFHeuristic(int h_type, bool use_cache)
    : Heuristic(use_cache), heuristic_type(h_type) {
...
int FFHeuristic::compute_ff_heuristic(const State &state) {
	...
	if(!g_use_metric || heuristic_type == PLAN_LENGTH)
        ...
        if(g_use_metric && heuristic_type == PLAN_COST)
            cost-= relaxed_plan.size();
	    return cost;
	}
    }
}
