...
class Action(object):
  ...
  def __init__(self, name, parameters, precondition, effect, ag, agt):
    self.name = name
  ...
  def pddl_rep(self):
    ...
    for pred in self.precondition + self.effect:
      if pred.name not in FORBIDDEN_PREDS:
        if self.agent[0] in pred.args:
          toadd.add(Predicate('K-ag-pred', [self.agent[0], 'pred--'+pred.name], False, False))
          #print "+" + str(pred)
        else:
          toadd.add(Predicate('K-pred', [self.agent[0], 'pred--'+pred.name], False, False))
          #print "-" + str(pred)
    ...
    for arg in arguments:
      ...
      if self.agent[0] != arg:
          self.precondition.append(Predicate("K-obj", [self.agent[0], arg], False, False))
...
class PlanningProblem(object):
  ...
  def parse_domain(self, domainfile):
    ...
    for action in self.actions:
      ...
      new_act = Action(act_name, up_params, pre_list, eff_list, agent, act.get('agent')[2])
