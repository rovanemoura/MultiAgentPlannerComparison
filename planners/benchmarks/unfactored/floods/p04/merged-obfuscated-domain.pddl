(define (domain floods)
  ...
  (:predicates (anon-have_water_sample-ugv3 ?r - usv ?a - area)
   ...
   (anon-have_water_sample-uav1 ?r - usv ?a - area)
   (anon-box_at-uav1 ?b - box ?r - ugv) (anon-have_picture-uav1 ?d - disaster)
   (anon-store_of-uav1 ?s - store) (anon-at-uav1 ?a - area)
   ...
   (anon-at-ugv1 ?a - area) (anon-have_water_sample-usv5 ?a - area)
  ...
  (:action anon-communicate_data-uav1 :parameters
   (?c - cdm ?d - disaster ?a1 - area ?a2 - area) :precondition
   (and (anon-at-uav1 ?a1) (cdm_at ?c ?a2) (anon-have_picture-uav1 ?d)
        (in_range ?a1 ?a2))
   :effect (and (not (anon-have_picture-uav1 ?d)) (communicated_data ?d)))

  (:action anon-take_picture-uav1 :parameters (?a - area ?d - disaster)
   :precondition (and (visible_from ?d ?a) (anon-at-uav1 ?a)) :effect
   (and (anon-have_picture-uav1 ?d)))

  (:action anon-navigate_uav-uav1 :parameters (?a1 - area ?a2 - area)
   :precondition (and (anon-at-uav1 ?a1)) :effect
   (and (not (anon-at-uav1 ?a1)) (anon-at-uav1 ?a2)))
