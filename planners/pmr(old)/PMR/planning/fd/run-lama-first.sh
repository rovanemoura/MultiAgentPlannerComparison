...
./$BASEDIR/src/fast-downward.py --plan-file $3  $1 $2  --search-options --heuristic "hlm,hff=lm_ff_syn(lm_rhw(reasonable_orders=true,lm_cost_type=2,cost_type=2))" --search "lazy_greedy(hff,hlm,preferred=[hff,hlm])" > $4

# ./src/translate/translate.py $1 $2
# ./src/preprocess/preprocess < output.sas
# ./src/search/downward --heuristic "hlm,hff=lm_ff_syn(lm_rhw(reasonable_orders=true,lm_cost_type=2,cost_type=2))" --search "lazy_greedy(hff,hlm,preferred=[hff,hlm])" --plan-file $3 < output > $4
# 
