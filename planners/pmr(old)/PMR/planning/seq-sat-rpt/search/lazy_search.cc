...
void LazySearch::initialize() {
    ...
    for (set<Heuristic *>::iterator it = hset.begin(); it != hset.end(); it++) {
        ...
        (*it)->evaluate(*g_initial_state); // Vidal: just to initialize it
    }
...
std::pair<State*, Plan> LazySearch::search(State new_state, vector<pair<int, int> > new_goals) { // Vidal
    reset(new_state, new_goals);
    FFHeuristic* ff = (FFHeuristic*) heuristics[0];
    ff->build_goals(new_goals);
    int iterations = 0;
    int result = IN_PROGRESS;
    while ( (result=step()) == IN_PROGRESS && iterations++ < node_limit) { }

    Plan plan_aux;
    if (result == FAILED) {
        return make_pair<State*, Plan>(NULL, plan_aux);
    }

    if (result == IN_PROGRESS) {
        State* last_state = new State(current_predecessor_buffer);
        search_space.trace_path(*last_state, plan_aux);
        return make_pair<State*, Plan>(last_state, plan_aux);
    } else {
        State* last_state = new State(current_state);
        search_space.trace_path(*last_state, plan_aux);
        return make_pair<State*, Plan>(last_state, plan_aux);
    }
