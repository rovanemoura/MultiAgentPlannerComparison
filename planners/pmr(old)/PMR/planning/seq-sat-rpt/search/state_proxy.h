...
public:
    ...
    mutable bool permanent; // Vidal

    StateProxy() : permanent(false) {
        state_data = 0;
    }

    ~StateProxy() { // Vidal
        if (permanent) {
            // std::cout << "~StateProxy" << std::endl;
            delete [] state_data;
        }
    }

    StateProxy(state_var_t *state_data_) : permanent(false) {
        state_data = state_data_;
    }

    explicit StateProxy(const State *state) : permanent(false) {
    ...
    void make_permanent() const {
        // std::cout << "Make permanent" << std::endl;
