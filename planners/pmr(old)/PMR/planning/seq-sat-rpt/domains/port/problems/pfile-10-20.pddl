(define (problem nil) (:domain port)
  (:objects ship9 ship8 ship7 ship6 ship5 ship4 ship3 ship2 ship1 ship0 - ship
             dock0 - dock
             pallet90 pallet89 pallet88 pallet87 pallet86 pallet85 pallet84 pallet83 pallet82 pallet81 pallet80 pallet79 pallet78 pallet77 pallet76 pallet75 pallet74 pallet73 pallet72 pallet71 pallet70 pallet69 pallet68 pallet67 pallet66 pallet65 pallet64 pallet63 pallet62 pallet61 pallet60 pallet59 pallet58 pallet57 pallet56 pallet55 pallet54 pallet53 pallet52 pallet51 pallet50 pallet49 pallet48 pallet47 pallet46 pallet45 pallet44 pallet43 pallet42 pallet41 pallet40 pallet39 pallet38 pallet37 pallet36 pallet35 pallet34 pallet33 pallet32 pallet31 pallet30 pallet29 pallet28 pallet27 pallet26 pallet25 pallet24 pallet23 pallet22 pallet21 pallet20 pallet19 pallet18 pallet17 pallet16 pallet15 pallet14 pallet13 pallet12 pallet11 - pallet-ship
             pallet10 pallet9 pallet8 pallet7 pallet6 pallet5 pallet4 pallet3 pallet2 pallet1 pallet0 - pallet-dock
             crate19 crate18 crate17 crate16 crate15 crate14 crate13 crate12 crate11 crate10 crate9 crate8 crate7 crate6 crate5 crate4 crate3 crate2 crate1 crate0 - crate
             level3 level2 level1 level0 - level
             hoist9 hoist8 hoist7 hoist6 hoist5 hoist4 hoist3 hoist2 hoist1 hoist0 - hoist
            )
  (:init
       (clear pallet11)
       (clear pallet12)
       (clear pallet13)
       (clear pallet14)
       (clear pallet15)
       (clear pallet16)
       (clear pallet17)
       (clear pallet18)
       (clear pallet19)
       (clear pallet20)
       (clear pallet21)
       (clear pallet22)
       (clear pallet23)
       (clear pallet24)
       (clear pallet25)
       (clear pallet26)
       (clear pallet27)
       (clear pallet28)
       (clear pallet29)
       (clear pallet30)
       (clear pallet31)
       (clear pallet32)
       (clear pallet33)
       (clear pallet34)
       (clear pallet35)
       (clear pallet36)
       (clear pallet37)
       (clear pallet38)
       (clear pallet39)
       (clear pallet40)
       (clear pallet41)
       (clear pallet42)
       (clear pallet43)
       (clear pallet44)
       (clear pallet45)
       (clear pallet46)
       (clear pallet47)
       (clear pallet48)
       (clear pallet49)
       (clear pallet50)
       (clear pallet51)
       (clear pallet52)
       (clear pallet53)
       (clear pallet54)
       (clear pallet55)
       (clear pallet56)
       (clear pallet57)
       (clear pallet58)
       (clear pallet59)
       (clear pallet60)
       (clear pallet61)
       (clear pallet62)
       (clear pallet63)
       (clear pallet64)
       (clear pallet65)
       (clear pallet66)
       (clear pallet67)
       (clear pallet68)
       (clear pallet69)
       (clear pallet70)
       (clear pallet71)
       (clear pallet72)
       (clear pallet73)
       (clear pallet74)
       (clear pallet75)
       (clear pallet76)
       (clear pallet77)
       (clear pallet78)
       (clear pallet79)
       (clear pallet80)
       (clear pallet81)
       (clear pallet82)
       (clear pallet83)
       (clear pallet84)
       (clear pallet85)
       (clear pallet86)
       (clear pallet87)
       (clear pallet88)
       (clear pallet89)
       (clear pallet90)
       (clear crate1)
       (clear crate19)
       (clear crate0)
       (clear crate16)
       (clear crate17)
       (clear pallet5)
       (clear crate13)
       (clear crate9)
       (clear crate3)
       (clear crate12)
       (clear crate14)
       (height crate19 level3)
       (on-dock crate19 crate18)
       (at crate19 dock0)
       (height crate18 level2)
       (on-dock crate18 crate6)
       (at crate18 dock0)
       (height crate17 level1)
       (on-dock crate17 pallet4)
       (at crate17 dock0)
       (height crate16 level3)
       (on-dock crate16 crate15)
       (at crate16 dock0)
       (height crate15 level2)
       (on-dock crate15 crate4)
       (at crate15 dock0)
       (height crate14 level1)
       (on-dock crate14 pallet10)
       (at crate14 dock0)
       (height crate13 level3)
       (on-dock crate13 crate11)
       (at crate13 dock0)
       (height crate12 level3)
       (on-dock crate12 crate10)
       (at crate12 dock0)
       (height crate11 level2)
       (on-dock crate11 crate5)
       (at crate11 dock0)
       (height crate10 level2)
       (on-dock crate10 crate2)
       (at crate10 dock0)
       (height crate9 level3)
       (on-dock crate9 crate8)
       (at crate9 dock0)
       (height crate8 level2)
       (on-dock crate8 crate7)
       (at crate8 dock0)
       (height crate7 level1)
       (on-dock crate7 pallet7)
       (at crate7 dock0)
       (height crate6 level1)
       (on-dock crate6 pallet1)
       (at crate6 dock0)
       (height crate5 level1)
       (on-dock crate5 pallet6)
       (at crate5 dock0)
       (height crate4 level1)
       (on-dock crate4 pallet3)
       (at crate4 dock0)
       (height crate3 level1)
       (on-dock crate3 pallet8)
       (at crate3 dock0)
       (height crate2 level1)
       (on-dock crate2 pallet9)
       (at crate2 dock0)
       (height crate1 level1)
       (on-dock crate1 pallet0)
       (at crate1 dock0)
       (height crate0 level1)
       (on-dock crate0 pallet2)
       (at crate0 dock0)
       (height pallet90 level0)
       (at pallet90 ship9)
       (height pallet89 level0)
       (at pallet89 ship9)
       (height pallet88 level0)
       (at pallet88 ship9)
       (height pallet87 level0)
       (at pallet87 ship9)
       (height pallet86 level0)
       (at pallet86 ship9)
       (height pallet85 level0)
       (at pallet85 ship9)
       (height pallet84 level0)
       (at pallet84 ship9)
       (height pallet83 level0)
       (at pallet83 ship9)
       (available hoist9)
       (assigned hoist9 ship9)
       (height pallet82 level0)
       (at pallet82 ship8)
       (height pallet81 level0)
       (at pallet81 ship8)
       (height pallet80 level0)
       (at pallet80 ship8)
       (height pallet79 level0)
       (at pallet79 ship8)
       (height pallet78 level0)
       (at pallet78 ship8)
       (height pallet77 level0)
       (at pallet77 ship8)
       (height pallet76 level0)
       (at pallet76 ship8)
       (height pallet75 level0)
       (at pallet75 ship8)
       (available hoist8)
       (assigned hoist8 ship8)
       (height pallet74 level0)
       (at pallet74 ship7)
       (height pallet73 level0)
       (at pallet73 ship7)
       (height pallet72 level0)
       (at pallet72 ship7)
       (height pallet71 level0)
       (at pallet71 ship7)
       (height pallet70 level0)
       (at pallet70 ship7)
       (height pallet69 level0)
       (at pallet69 ship7)
       (height pallet68 level0)
       (at pallet68 ship7)
       (height pallet67 level0)
       (at pallet67 ship7)
       (available hoist7)
       (assigned hoist7 ship7)
       (height pallet66 level0)
       (at pallet66 ship6)
       (height pallet65 level0)
       (at pallet65 ship6)
       (height pallet64 level0)
       (at pallet64 ship6)
       (height pallet63 level0)
       (at pallet63 ship6)
       (height pallet62 level0)
       (at pallet62 ship6)
       (height pallet61 level0)
       (at pallet61 ship6)
       (height pallet60 level0)
       (at pallet60 ship6)
       (height pallet59 level0)
       (at pallet59 ship6)
       (available hoist6)
       (assigned hoist6 ship6)
       (height pallet58 level0)
       (at pallet58 ship5)
       (height pallet57 level0)
       (at pallet57 ship5)
       (height pallet56 level0)
       (at pallet56 ship5)
       (height pallet55 level0)
       (at pallet55 ship5)
       (height pallet54 level0)
       (at pallet54 ship5)
       (height pallet53 level0)
       (at pallet53 ship5)
       (height pallet52 level0)
       (at pallet52 ship5)
       (height pallet51 level0)
       (at pallet51 ship5)
       (available hoist5)
       (assigned hoist5 ship5)
       (height pallet50 level0)
       (at pallet50 ship4)
       (height pallet49 level0)
       (at pallet49 ship4)
       (height pallet48 level0)
       (at pallet48 ship4)
       (height pallet47 level0)
       (at pallet47 ship4)
       (height pallet46 level0)
       (at pallet46 ship4)
       (height pallet45 level0)
       (at pallet45 ship4)
       (height pallet44 level0)
       (at pallet44 ship4)
       (height pallet43 level0)
       (at pallet43 ship4)
       (available hoist4)
       (assigned hoist4 ship4)
       (height pallet42 level0)
       (at pallet42 ship3)
       (height pallet41 level0)
       (at pallet41 ship3)
       (height pallet40 level0)
       (at pallet40 ship3)
       (height pallet39 level0)
       (at pallet39 ship3)
       (height pallet38 level0)
       (at pallet38 ship3)
       (height pallet37 level0)
       (at pallet37 ship3)
       (height pallet36 level0)
       (at pallet36 ship3)
       (height pallet35 level0)
       (at pallet35 ship3)
       (available hoist3)
       (assigned hoist3 ship3)
       (height pallet34 level0)
       (at pallet34 ship2)
       (height pallet33 level0)
       (at pallet33 ship2)
       (height pallet32 level0)
       (at pallet32 ship2)
       (height pallet31 level0)
       (at pallet31 ship2)
       (height pallet30 level0)
       (at pallet30 ship2)
       (height pallet29 level0)
       (at pallet29 ship2)
       (height pallet28 level0)
       (at pallet28 ship2)
       (height pallet27 level0)
       (at pallet27 ship2)
       (available hoist2)
       (assigned hoist2 ship2)
       (height pallet26 level0)
       (at pallet26 ship1)
       (height pallet25 level0)
       (at pallet25 ship1)
       (height pallet24 level0)
       (at pallet24 ship1)
       (height pallet23 level0)
       (at pallet23 ship1)
       (height pallet22 level0)
       (at pallet22 ship1)
       (height pallet21 level0)
       (at pallet21 ship1)
       (height pallet20 level0)
       (at pallet20 ship1)
       (height pallet19 level0)
       (at pallet19 ship1)
       (available hoist1)
       (assigned hoist1 ship1)
       (height pallet18 level0)
       (at pallet18 ship0)
       (height pallet17 level0)
       (at pallet17 ship0)
       (height pallet16 level0)
       (at pallet16 ship0)
       (height pallet15 level0)
       (at pallet15 ship0)
       (height pallet14 level0)
       (at pallet14 ship0)
       (height pallet13 level0)
       (at pallet13 ship0)
       (height pallet12 level0)
       (at pallet12 ship0)
       (height pallet11 level0)
       (at pallet11 ship0)
       (available hoist0)
       (assigned hoist0 ship0)
       (height pallet10 level0)
       (at pallet10 dock0)
       (height pallet9 level0)
       (at pallet9 dock0)
       (height pallet8 level0)
       (at pallet8 dock0)
       (height pallet7 level0)
       (at pallet7 dock0)
       (height pallet6 level0)
       (at pallet6 dock0)
       (height pallet5 level0)
       (at pallet5 dock0)
       (height pallet4 level0)
       (at pallet4 dock0)
       (height pallet3 level0)
       (at pallet3 dock0)
       (height pallet2 level0)
       (at pallet2 dock0)
       (height pallet1 level0)
       (at pallet1 dock0)
       (height pallet0 level0)
       (at pallet0 dock0)
       (next level2 level3)
       (next level1 level2)
       (next level0 level1))
  (:goal (and 
       (on-ship crate19 pallet86 ship9)
       (on-ship crate18 pallet41 ship3)
       (on-ship crate17 pallet15 ship0)
       (on-ship crate16 pallet42 ship3)
       (on-ship crate15 pallet76 ship8)
       (on-ship crate14 pallet65 ship6)
       (on-ship crate13 crate8 ship1)
       (on-ship crate12 crate11 ship1)
       (on-ship crate11 pallet19 ship1)
       (on-ship crate10 pallet73 ship7)
       (on-ship crate9 pallet68 ship7)
       (on-ship crate8 crate3 ship1)
       (on-ship crate7 pallet22 ship1)
       (on-ship crate6 pallet30 ship2)
       (on-ship crate5 pallet77 ship8)
       (on-ship crate4 pallet43 ship4)
       (on-ship crate3 pallet20 ship1)
       (on-ship crate2 pallet26 ship1)
       (on-ship crate1 pallet29 ship2)
       (on-ship crate0 pallet69 ship7))))