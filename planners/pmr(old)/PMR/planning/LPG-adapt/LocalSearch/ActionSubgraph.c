...
#ifdef __MY_OUTPUT__
    printf("\n Curr  MAX_PLAN_LENGTH %d ",  MAX_PLAN_LENGTH);
...
int insert_action_in_vectlevel (int act_pos, int level)
{
  ...
  **/
  ...
  if(GpG.input_plan_actions[o_pos]>0  ) // Check if act_pos is an input plan action actually not in the action graph 
    {
      GpG.input_plan_actions[o_pos]--; 
  ...
  if(vectlevel[level]->action.input_plan_act>0)
    {
      ...
      if(GpG.input_plan_actions[o_pos]<0)
	{ /* I have to assign the input_plan_act flag to another action node */
	  GpG.num_new_actions--;
	  
	  for(cel=0; cel< GpG.curr_plan_length; cel++){
	    int pos=GET_ACTION_POSITION_OF_LEVEL (cel);
	    if(gef_conn[pos].orig_or_position ==o_pos && vectlevel[cel]->action.input_plan_act==0 )
	      {
		vectlevel[cel]->action.input_plan_act=1;
		break;
	      }
	  }
	  if( cel== GpG.curr_plan_length && DEBUG2)
...
void restart_MetricMinimizeCost ()
{
  ...
  do
	...
	num_act = MY_RANDOM % ((int) ceil (fabs(GpG.best_cost)));
	...
	if (vectlevel[level]->num_actions)
	  {
	    ...
	    sum += fabs(get_action_cost (pos_act, level, NULL));
	    /** 
