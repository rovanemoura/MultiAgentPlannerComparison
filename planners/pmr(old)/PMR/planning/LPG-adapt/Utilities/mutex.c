...
//#define __TEST_LOWMEMORY_MUTEX__
...
int are_mutex_ops (int a, int b)
{
  ...
  call++;
   ...
   if ((gef_conn[a].act_type == START_SPLITTED) 
      || (gef_conn[a].act_type == END_SPLITTED)
      || (gef_conn[b].act_type == START_SPLITTED) 
      || (gef_conn[b].act_type == END_SPLITTED)) 
    {
      if (Econstraint_type(a, -1, b, -1) != 0)
	return TRUE;
      else
	return FALSE;
    }
  ...
  if (!GpG.lowmemory)
...
#ifdef __TEST_LOWMEMORY_MUTEX__
      GpG.lowmemory = TRUE;
      
      k = are_mutex_ops(a, b);
      if (k != i)
	{
	  printf("\n\nERROR [%d]: actions", call);
	  printf("\n\t[%d] ", a);
	  print_op_name(a);
	  printf("\n\t[%d] ", b);
	  print_op_name(b);
	  printf("\n are %s with lowmemory", (k != 0)?"MUTEX":"NOT MUTEX");
	  printf("\n are %s without lowmemory", (i != 0)?"MUTEX":"NOT MUTEX");
	}
    ...
    
    {
      ...
      /* NEW: controllo PC vs effetti additivi
       */
	...
	for (i = 0; i < gef_conn[a].num_PC; i++)
	  {
	    if (gef_conn[a].PC[i] < 0)
	      continue;
	    if (is_fact_in_additive_effects(b, gef_conn[a].PC[i]))
	      return TRUE;
	  }

	for (i = 0; i < gef_conn[b].num_PC; i++)
	  {
	    if (gef_conn[b].PC[i] < 0)
	      continue;
	    if (is_fact_in_additive_effects(a, gef_conn[b].PC[i]))
	      return TRUE;
	  }
	
      } else {
      	
	if (gef_conn[a].sf != NULL)
	  {
	    for (i = 0; i < gef_conn[a].sf->num_PC_overall; i++) 
	      {
		if (gef_conn[a].sf->PC_overall[i] < 0)
		  continue;
		if (is_fact_in_additive_effects(b, gef_conn[a].sf->PC_overall[i]) ||
		    is_fact_in_additive_effects_start(b, gef_conn[a].sf->PC_overall[i]))
		  return TRUE;
	      }
	  }

	if (gef_conn[b].sf != NULL)
	  {
	    for (i = 0; i < gef_conn[b].sf->num_PC_overall; i++)
	      {
		if (gef_conn[b].sf->PC_overall[i] < 0)
		  continue;
		if (is_fact_in_additive_effects(a, gef_conn[b].sf->PC_overall[i]) ||
		    is_fact_in_additive_effects_start(a, gef_conn[b].sf->PC_overall[i]))
		  return TRUE;
	      }
	  }
      }
      ...
      /* NEW: controllo or_PC vs effetti 
       */
      for (i = 0; i < gef_conn[a].num_or_PC; i++)
	{
	  if (gef_conn[a].or_PC[i] < 0)
	    continue;
	  if (is_fact_in_additive_effects(b, gef_conn[a].or_PC[i]) || 
	      is_fact_in_delete_effects(b, gef_conn[a].or_PC[i]) ||
	      is_fact_in_additive_effects_start(b, gef_conn[a].or_PC[i]) ||
	      is_fact_in_delete_effects_start(b, gef_conn[a].or_PC[i]))
	    return TRUE;
	}
      
      for (i = 0; i < gef_conn[b].num_or_PC; i++)
	{
	  if (gef_conn[b].or_PC[i] < 0)
	    continue;
	  if (is_fact_in_additive_effects(a, gef_conn[b].or_PC[i]) ||
	      is_fact_in_delete_effects(a, gef_conn[b].or_PC[i]) ||
	      is_fact_in_additive_effects_start(a, gef_conn[b].or_PC[i]) ||
	      is_fact_in_delete_effects_start(a, gef_conn[b].or_PC[i]))
	    return TRUE;
	}
...
are_goal_reachable_and_non_mutex (void)
{
  ...
  for (i = 0; i < ggoal_state.num_F; i++)
    ...
    if (ggoal_state.F[i]>=0 && (gft_conn[ggoal_state.F[i]].level < 0 || ( Hvar.init_facts_array[ggoal_state.F[i]] &&  Hvar.init_facts_array[ggoal_state.F[i]]->best_act<0 && Hvar.init_facts_array[ggoal_state.F[i]]->best_act!=INITIAL_ACTION )) )
      {
	...
	printf("\n Unreacheble Goal %d :  ", ggoal_state.F[i]);
	print_ft_name(ggoal_state.F[i]);
  ...
  **/
  ...
  do
	...
	if(!GET_BIT (applied, i)) 
	  if(num_prec_vector[i]==0 && (gef_conn[i].position >= 0))	 
...
void calc_mutex_derived(void) 
{
  ...
  for (i = start; i < gnum_ft_conn; i++)
    {
      build_activation_set_mutex_intersection(i, FT_FT_mutex[i]);
...
calc_mutex_ops (void)
{
  ...
  int DPre[gnum_ft_conn];
  int Eff[gnum_ft_conn];
  ...
  if (!GpG.lowmemory)
    {
	 ...
	 B, in tal caso A e B sono mutex poiche' vi e' interferenza e la matrice delle mutex viene aggiornata */
       for(i=0;i<gnum_ef_conn;i++)
	{
	  /* NEW: controllo le mutex dovute a fatti che comparivano nei nodi OR
	   *      precondizioni (che potrebbero essere stati rimossi da questa 
	   *      azione durante l'instanziazione)
	   */
	  for (j = 0; j < gef_conn[i].num_or_PC; j++) {
	    for(k=0;k<gft_conn[gef_conn[i].or_PC[j]].num_D;k++)
	      {
		if ( gft_conn[gef_conn[i].or_PC[j]].D[k] >=0)
		  {
		    if(GET_EF_EF_MX_BIT(i, gft_conn[gef_conn[i].or_PC[j]].D[k]))
		      continue;
		    if (DEBUG2)
		      total_ef_ef_mutex++;
		    
		    SET_EF_EF_MX_BIT (i, gft_conn[gef_conn[i].or_PC[j]].D[k]);
		  } 
	      }
	    
	    for(k=0;k<gft_conn[gef_conn[i].or_PC[j]].num_A;k++)
	      {
		if ( gft_conn[gef_conn[i].or_PC[j]].A[k] >=0)
		  {
		    if(GET_EF_EF_MX_BIT(i, gft_conn[gef_conn[i].or_PC[j]].A[k]))
		      continue;
		    if (DEBUG2)
		      total_ef_ef_mutex++;
		    
		    SET_EF_EF_MX_BIT (i, gft_conn[gef_conn[i].or_PC[j]].A[k]);
		  } 
	      }
	  }
	  ...
	  for(j=0;j<gef_conn[i].num_A;j++)
       ...
       if (GpG.derived_predicates)
	 {
	   for (i = 0; i < ( gnum_ef_conn - 1); i++)
	     {
	       
	       build_DPre(DPre, i);
	       
	       for (j = i+1; j < gnum_ef_conn; j++)
		 {
		   ...
		   for (l = 0; l < gnum_ft_block; l++)
		     {
		       if (DPre[l] & Eff[l])
			 {
			   SET_EF_EF_MX_BIT(i, j);
			   if (DEBUG2)
			     printf("\nNew mutex found (%d <-> %d) during derived preconditions analysis.", i, j);
			 }
		     }
		 }
	     }
...
Bool apply_best_increase_for_var(int num_var,float *vmax)
{
  ...
  if ((vmax[num_var] - old_value) >= MAX_APPROX)
    return TRUE;
...
Bool apply_best_decrease_for_var(int num_var,float *vmax,float *vmin)
{
  ...
  if ((old_value - vmin[num_var]) >= MAX_APPROX)
    return TRUE;
  ...
  for(i=0;i<gnum_comp_var;i++)
	...
	case LESS_THAN_OR_EQUAL_OP:
	   ...
	   */
	    ...
	    if (/* GET_BIT(relevant_vars, i) 
		  &&*/ (Hvar.ri_best_decrease_for_compvar[i] >=0))
	      changed=TRUE;
	    
	  /*
	    if ((GET_BIT(relevant_vars, gcomp_var[i].first_op)
	    && (Hvar.ri_best_decrease_for_compvar[gcomp_var[i].first_op] >= 0))
	    || (GET_BIT(relevant_vars, gcomp_var[i].second_op)
	    && (Hvar.ri_best_increase_for_compvar[gcomp_var[i].second_op] >= 0)))
	    changed = TRUE;
	  */
	  break;
	case GREATER_THAN_OP:
	case GREATER_OR_EQUAL_OP:
	   ...
	   */
	    ...
	    if (/*GET_BIT(relevant_vars, i) 
		  && */ (Hvar.ri_best_increase_for_compvar[i]>=0))
	      changed=TRUE;

	  /* 
	     if ((GET_BIT(relevant_vars, gcomp_var[i].first_op)
	     && (Hvar.ri_best_increase_for_compvar[gcomp_var[i].first_op] >= 0))
	     || (GET_BIT(relevant_vars, gcomp_var[i].second_op)
	     && (Hvar.ri_best_decrease_for_compvar[gcomp_var[i].second_op] >= 0)))
	     changed = TRUE;
	  */
	  break;
	...
	case EQUAL_OP:
	   ...
	   */
	    ...
	    if (/*GET_BIT(relevant_vars, i) 
		  &&*/ (Hvar.ri_best_increase_for_compvar[i]>=0 || Hvar.ri_best_decrease_for_compvar[i]>=0))
	      changed=TRUE;
	  /*
	    if ((GET_BIT(relevant_vars, gcomp_var[i].first_op)
	    && ((Hvar.ri_best_decrease_for_compvar[gcomp_var[i].first_op] >= 0) 
