  ...
  for (i = 0; i < gft_conn[dp_fct].num_dp_A; i++) {
	...
	**/
	...
	if (GpG.derived_pruning_on)
	  {
	    /*
	      Controllo se questo fatto è mutex con l'azione data
	      **
	    ...
	    */
	    ...
	    /*
	      Il costo della tupla è pari al max dei costi dei fatti
	       che la compongono
	    */
	    ...
	    /*
	      Scarto le tuple che costano più della
	...
	**/
	...
	if (GpG.derived_pruning_on)
	{
	  ...
	  */
	  ...
	  /*
	    Scarto le che costano più della
	    tupla migliore sinora trovata
	  */
	  if (temp_tuple_cost > (best_pc_cost * GpG.pruning_weight))
	    {
 ...
 * Scopo:  Sceglie l'insieme di precondizioni più facile da supportare in base
...
indexed_vect_list *choose_best_facts_set_fast(path_set *path, int *num_pc, int level, int to_level, float *best_cost, int *max_depth) {
  ...
  for (i = 1; i <= path->size; i++) {
    ...
    for (tmp = path->tuple_set[i]; tmp; tmp = tmp -> next) {
      ...
      /**
	 Se tmp->op è minore di zero significa che ho scartato questo insieme
	 **
	...
	continue;
	 ...
	 in questione non è propagabile fino a to_level, lo scarto
	 **
	...
	printf("\n\nTUPLE => COST : %.2f\n", temp_cost);
      ...
      /**
	 Se il costo della tupla è inferiore al migliore trovato, allora scelgo questa tupla.
	 Se il costo è uguale al migliore, scelgo in base alla distanza dal fatto derivato da
...
int choose_fact_to_support(indexed_vect_list *tuple, int num_f, int level) {
  ...
  /**
     Se è attivo il flag GpG.choose_random_fact_from_tuple (default) scelgo a caso
