 ...
 **
 * Init constants used for handling a bit table
 **/
int init_bit_table_const(unsigned long int max_size, int *n_bit, int *base, int *bit_row_size) {
   ...
   **
   * Evaluate the number of integer to allocate for each row
   **/
  (*bit_row_size)--;
  ...
  /**
   * bit-mask per il recupero di un elemento da un blocco 
   * della hash
   **
   * bit-mask for retrieving an element from a block 
   * of this hash table.
   **/
 ...
 **
 * Init the matrix that implement a row of an hash table
 **/
 ...
 **
 * Set the bit in the position <op, adr> of a bit table
 **/
 ...
 **
 * Reset the bit in the position <op, adr> of a bit table
 **/
 ...
 **
 * Check the bit in the position <predicate, adr> of a bit table
 **/
...
/**
 * Ritorna un blocco di bit (int) contenuto in una bit-table
 **
 * Return a bit block (an int) from a bit table
 **/
 ...
 **
 * Add to the operator "op" some implicit preconditions. They say that a parameter can not
 * assume the same value of another parameter if it can result in a potential contraddiction.
 **/
void add_implicit_preconds(Operator *op) {
  ...
  /**
   * Per ogni effetto dell'operatore
   **
   * For each operator's effect
   **/
  for (ef1 = op->effects; ef1; ef1 = ef1->next) {
	...
	for (f2 = (ef1==ef2)? f1->next : ef2->effects; f2; f2 = f2->next) {
	  ...
	  if ((f1->fact.predicate == f2->fact.predicate)
		...
		/**
		 * E' lo stesso parametro --> non posso imporre disugualianze
		 **
		...
		/**
		 * Genero un nodo di disugualianza
		 **
		 * Build not-eq node
		 **/		
		...
		/**
		 * Aggiungo questo nodo alle precondizioni dell'operatore
		 **
 ...
 **
 * Extract the timed precondition from an action and place it in the vector timed_PC
 **/
...
void add_composite_vars (int from_ef_conn, int to_ef_conn)
{
  ...
  for (i = 0; i < gnum_comp_var; i++)
    {
      gcomp_var[i].increased_by = gcomp_var[i].decreased_by = NULL;
  ...
  set_numeric_flag(from_ef_conn, to_ef_conn);
...
// vefifica condizioni numeriche
		case BIN_COMP:
			...
			case OVER_ALL_CONN:
				...
				cef->sf->PC_overall = (int *) realloc(cef->sf->PC_overall, cef->sf->num_PC_overall + 1);
				cef->sf->PC_overall[cef->sf->num_PC_overall++] = -index_in_cvars_of_expression(n->sons, cef->ef);
		  		break;
			case AT_END_CONN:
				if (cef->sf == NULL)
					cef->sf = new_SpecialFacts();
				cef->sf->PC_end = (int *) realloc(cef->sf->PC_end, cef->sf->num_PC_end + 1);
				cef->sf->PC_end[cef->sf->num_PC_end++] =  -index_in_cvars_of_expression(n->sons, cef->ef);
				break;
			default:
				cef->PC = (int *) realloc(cef->PC, cef->num_PC + 1);
...
// vefifica effetti numerici
		...
		case ASSIGN_CONN:
			if (e->connective == AT_START_CONN) {
				if (cef->sf == NULL)
					cef->sf = new_SpecialFacts();
				cef->sf->A_start = (int *) realloc(cef->sf->A_start, cef->sf->num_A_start + 1);
	...
	for (; e; e = e->next) {
		if ((e->connective == AT_START_CONN) && (is_bool_fact(e->sons))) {
			...
			if (pos != -1) {
				if (cef->sf == NULL)
					cef->sf = new_SpecialFacts();
				cef->sf->D_start = (int *) realloc(cef->sf->D_start, cef->sf->num_D_start + 1);
  ...
  for (i = from_ef_conn; i < to_ef_conn; i++)
    {
      ...
      if (gop_conn[gef_conn[i].op].action->norm_operator != NULL)
	num_preconds = gop_conn[gef_conn[i].op].action->norm_operator->numeric;
      else
	num_preconds = gop_conn[gef_conn[i].op].action->pseudo_action->numeric;

      preconds = gef_conn[i].plop->preconds;
      

      //MODIFICATO: salto tutto e chiamo la funzione add_durative_preconds
      if (FALSE && preconds != NULL)
	{
	...
	add_durative_preconditions(i);

	
    
      if (num_preconds != NULL)
	{
	  for (n = num_preconds; n; n = n->next)
	    {
	      /* preco numerica                    :  aggiungere in PC, PC_overall, PC_end */
	      /* se incontro uno di questi connective, passo ad aggiungere le composite vars */
	      if (n->connective != BIN_COMP)
		printf("\n\nWarning: found boolean precondition in \"numeric\" PlNode");
	      
	      e = n->sons;
	      switch (n->is_start_end_ovr)
		{
		case IS_OVERALL:
...
calc_duration_of_actions (int from_ef_conn, int to_ef_conn)
{
  ...
  for (i = from_ef_conn; i < to_ef_conn; i++)
      ...
      if (duration)
	{
	  ...
	  gef_conn[i].duration = eval_comp_var (&gcomp_var[ind], ind, gcomp_var_value);
...
void cvar_modified_by_condef_conn(CondEfConn *cef)
{
	...
	if (cef->sf == NULL) {
		size = cef->num_A;
		fact_array =  (int *) malloc(size * sizeof(int));
...
// salva il valore della funzione di ottimizzazione dopo l'applicazione degli effetti numerici
		if (goptimization_exp >= 0)
			cef->cost = eval_comp_var(&gcomp_var[goptimization_exp], goptimization_exp, gcomp_var_value) - opt_funct_before;
		else
			cef->cost = 1.0;
...
Bool cvar_modified_by_ef_conn(EfConn *ef)
{
  ...
  for (i = 0; i < size; i++) 
      ...
      if (fact_array[i] < 0) 
	{
	  ...
	  if (gcomp_var[gve->first_op].in_metric)
	    {	    
	      /*
	  ...
	  switch(gve->l_operator)
	  {
          case INCREASE_OP:
	    if (gcomp_var[gve->second_op].l_operator != FIX_NUMBER
...
void calc_cost_of_actions (int from_ef_conn, int to_ef_conn)
{
  ...
  */
  ...
  for (i = from_ef_conn; i < to_ef_conn; i++)
      ...
      if (gef_conn[i].is_numeric)
	{
	  ...
	  */
	  ...
	  else if (!metric_affected)
	    {
	      /*
		Se l'azione non modifica la metrica
		
		If metric is not affected by this action 
	      */
	  ...
	  /*
	    Ripristino lo stato delle variabili numeriche prima di passare alla prossima azione
	...
	gef_conn[i].cost *= -1;
      ...
      if (fabsf ( fabsf(gef_conn[i].cost) - MIN_ACTION_COST ) < MIN_VALUE)
	gef_conn[i].cost = MIN_ACTION_COST;
    }
  ...
  if (DEBUG2)
    {
      for (i = from_ef_conn; i < to_ef_conn; i++)
	printf("\n\nAzione : %d : %s : COST : %.2f", i, print_op_name_string(i, temp_name), 
	       gef_conn[i].cost);
    }
...
apply_numeric_effects_of_efconn (int index)
{
  ...
  if (gcomp_var_value_start == NULL)
    {
      gcomp_var_value_start = (float *)calloc(gnum_comp_var, sizeof(float));
      value_start_size = gnum_comp_var; 
    }

  if (value_start_size < gnum_comp_var)
    {
      gcomp_var_value_start = (float *)realloc(gcomp_var_value_start, gnum_comp_var * sizeof(float));
      value_start_size = gnum_comp_var; 
    }
  ...
  if (ef->sf != NULL)
	...
	if (*p < 0)
	  eval_comp_var (&gcomp_var_effects[-(*p)], -(*p), gcomp_var_value_before);
    }

  memcpy(gcomp_var_value_start, gcomp_var_value, gnum_comp_var * sizeof(float));
  
  // applica gli effetti at end
  for (p = ef->A; p < &ef->A[ef->num_A]; p++)
    if (*p < 0)
      eval_comp_var (&gcomp_var_effects[-(*p)], -(*p), gcomp_var_value_start);
...
int scan_all_cnum_vars(CompositeNumVar *vect, CompositeNumVar *cv, int num, float cv_value) {
  ...
  for (i = 0; i < num; i++) {
    tmp = &vect[i];
    if (tmp->l_operator == cv->l_operator
	&& tmp->first_op == cv->first_op
	&& tmp->second_op == cv->second_op) {

      if (cv->l_operator == FIX_NUMBER) {
...
index_in_cvars_of_expression (PlNode * pln, int ind)
{
  ...
  cvar =  new_CompositeNumVar(MUL_OP);
  ...
  switch (pln->connective)
    ...
    case MAXIMIZE_CONN:
      ...
      cvar->l_operator = op_from_conn (pln->connective);
	...
	|| gcomp_var[cvar->second_op].in_metric;
      cvar_value = eval_comp_var (cvar, -1, gcomp_var_value);
	...
	cvar_value = eval_comp_var (cvar, cvar->first_op, gcomp_var_value);
...
float
eval_comp_var (CompositeNumVar * cv, int index, float *values)
{
  float tmp, cv_value;
  ...
  switch (cv->l_operator)
    ...
    case MUL_OP:
      cv_value = eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op, values) *	eval_comp_var (&(gcomp_var[cv->second_op]), cv->second_op, values);
      break;
    case DIV_OP:
      tmp = eval_comp_var (&(gcomp_var[cv->second_op]), cv->second_op, values);
      if (tmp == 0)
	printf ("\n\nWARNING: Division by zero in eval_comp_var\n\n");
      cv_value = eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op, values) / tmp;
      break;
    case PLUS_OP:
      cv_value = eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op, values) +	eval_comp_var (&(gcomp_var[cv->second_op]), cv->second_op, values);
      break;
    case MINUS_OP:
      ...
      break;
    case UMINUS_OP:
      cv_value = -eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op, values);
      break;
    case INCREASE_OP:
      index = cv->first_op;
      cv_value = eval_comp_var (&(gcomp_var[cv->first_op]), cv->first_op, values) + eval_comp_var (&(gcomp_var[cv->second_op]), cv->second_op, values);
    ...
    case ASSIGN_OP:
      index = cv->first_op;
      cv_value = eval_comp_var (&(gcomp_var[cv->second_op]),cv->second_op, values);
      break;
    case MINIMIZE_OP:
      index = cv->first_op;
      cv_value = eval_comp_var (&(gcomp_var[cv->first_op]),cv->first_op, values);
  ...
  while (to_check)
    {
      ...
      if ((to_check->l_operator == cv->l_operator)
	  && (to_check->first_op == cv->first_op)
	  && (to_check->second_op == cv->second_op)) {
	if (to_check->l_operator == FIX_NUMBER) {
...
copy_compvar (CompositeNumVar * dest, CompositeNumVar * src)
{
  ...
  dest->l_operator = src->l_operator;
...
get_numvar_index_of_fn_head (PlNode * p, int index)
{
  ...
  **/
  ...
  gnum_fullnum_blocks = (gnum_fullnum_initial >> 5) + 1;
...
{ 
  if (ncvar == -1)
    return;
  switch (gcomp_var[ncvar].l_operator )
    {
    case MUL_OP:
    case DIV_OP:
    case MINUS_OP:
...
propagate_inertias (void)
{
  ...
  for (i = 0; i < gnum_comp_var; i++)
	...
	case LESS_THAN_OP:
	case LESS_THAN_OR_EQUAL_OP:
	  cv->l_operator= FIX_NUMBER;
	  cv->first_op = -1;
	  cv->second_op = -1;
	  GCOMP_VAR_VALUE(i)= eval_comp_var (cv, i, gcomp_var_value);
...
int get_num_of_effects_of (EfConn *ef, TimeSpec ts, Bool is_additive)
{
  if (ef->plop)
    {
      if (ts == AT_START_TIME)
	{
	  if (is_additive)
	    return ef->plop->num_effects_start;
	  else
	    return ef->plop->num_deleffects_start;
	}
      if (ts == AT_END_TIME)
	return ef->plop->num_numeric_effects_end;
      printf ("\n\nor AT_START neither AT_END!!!\n\n");
      exit (1);
    }
  ef->plop = search_name_in_plops (gop_conn[ef->op].action->name);
  if (!ef->plop)
    {
      printf ("\n\nop not found?!!!\n\n");
      exit (1);
  ...
  if (!ef->plop -> is_odd)
    ...
    if (!ef->sf)
      ef->sf = new_SpecialFacts ();

  return get_num_of_effects_of (ef, ts, is_additive);
}


int get_num_of_numeric_preconds_of (EfConn *ef, TimeSpec ts)
{
  NormOperator *no;
  PseudoAction *pa;
  PlNode *p;
  ...
  if (gop_conn[ef->op].action->norm_operator)
    {
      no = gop_conn[ef->op].action->norm_operator;

      for (p = no->numeric; p; p = p->next)
	{
	  if (p->connective != BIN_COMP)
	    {
	      printf("\n\nWarning: non BIN_COMP node in norm_operator->numeric");
	      continue;
	    }
	  if (((ts == AT_START_TIME) && (p->is_start_end_ovr == IS_AT_START))
	      || ((ts == AT_END_TIME) && (p->is_start_end_ovr == IS_AT_END))
	      || ((ts == OVER_ALL_TIME) && (p->is_start_end_ovr == IS_OVERALL)))
	    res++;
	}
    }
  else
    {
      pa = gop_conn[ef->op].action->pseudo_action;

      for (p = pa->numeric; p; p = p->next)
	{
	  if (p->connective != BIN_COMP)
	    {
	      printf("\n\nWarning: non BIN_COMP node in norm_operator->numeric");
...
int get_num_of_preconds_of (EfConn *ef, TimeSpec ts)
{
  int res = 0;
  ...
  if (ef->plop)
	...
	case AT_START_TIME:
	  break;
	case AT_END_TIME:
	  res =  ef->plop->num_preconds_end;
	  break;
	case OVER_ALL_TIME:
	  res =  ef->plop->num_preconds_overall;
	  break;
	default:
      ...
      res += get_num_of_numeric_preconds_of (ef, ts);
    ...
    {
      ef->plop =
	search_name_in_plops (gop_conn[ef->op].action->name);
      if (!ef->plop)
	{
	  printf ("\n\nop not found?!!!\n\n");
	  exit (1);
	...
	if (!ef->sf)
      ...
      return get_num_of_preconds_of (ef, ts);
    }
}
...
{
  unsigned long  out;
  out = (cv->l_operator + abs (cv->first_op) * 2563 + abs (cv->second_op) * 2563 * 2563);
  return ((out % BIGPRIME) % HASH_SIZE);
  //  return (((cv->l_operator + abs (cv->first_op) * 2563 + abs (cv->second_op) * 2563 * 2563) % BIGPRIME) % HASH_SIZE);
}
...
{
  int   k, b, bext, to_check, temp,  y;
  register int i, j , tempext, yext;
  Bool is_star_val;
	  ...
	  if (tempext & FIRST_1)
	    {
	      ...
	      if (gef_conn[i].dur_var_index > 0)
		{
		  set_rvals_for_cvar(i, gef_conn[i].dur_var_index);
		}
	  ...
	  if (tempext & FIRST_1)
		...
		if (gef_conn[i].A[j] < 0)	   
		  {   
		    ...
		    if ((gcomp_var_effects[-gef_conn[i].A[j]].l_operator == INCREASE_OP)
			|| (gcomp_var_effects[-gef_conn[i].A[j]].l_operator == DECREASE_OP))
		      {
		...
		for (j = 0; j < gef_conn[i].sf->num_A_start; j++)
		  ...
		  if (gef_conn[i].sf->A_start[j] < 0)
			...
			continue;
		      ...
		      if ((gcomp_var_effects[-gef_conn[i].sf->A_start[j]].l_operator == INCREASE_OP)
			  || (gcomp_var_effects[-gef_conn[i].sf->A_start[j]].l_operator == DECREASE_OP))
			{
			  insert_bit_in_bit_table(lstar_vals, i, to_check);
			  is_star_val=TRUE;
			}
...
Bool is_time_var_in_compvar(int ncvar)
{
  ...
  switch (gcomp_var[ncvar].l_operator )
    ...
    case UMINUS_OP:
    case PLUS_OP:
      if (is_time_var_in_compvar(gcomp_var[ncvar].first_op)
	  ||
	  is_time_var_in_compvar(gcomp_var[ncvar].second_op)) 
	{
#ifdef __CONT_EFF__
	  gcomp_var[ncvar].temp_value = 1.0;
#endif
	  return TRUE;
	}
      return FALSE;
...
Bool is_time_var_in_compvar_effects(int ncvar)
{
  ...
  switch (gcomp_var_effects[ncvar].l_operator )
    {
    case INCREASE_OP:
    case DECREASE_OP:
      return 
...
#ifdef __CONT_EFF__
	gcomp_var_effects[i].temp_value = 1.0;
#endif
...
void index_duration_exps (int from_ef_conn, int to_ef_conn)
{
  ...
  for (i = from_ef_conn; i < to_ef_conn; i++)
      ...
      if (duration)
	{
	  ...
	  gef_conn[i].isdurative = TRUE;
	}
      else 
	gef_conn[i].isdurative = FALSE;
    }
}
...
void make_numgoal_state(PlNode *list)
{
  ...
  /**
     Aggiorno il numero di blocchi per i bitarray delle variabili numeriche
     **
     Update the number of blocks for numeric variables bitarrays
  **/
  gnum_block_compvar = gnum_comp_var / 32 + 1;
...
void set_cost_and_time_coeffs (void)
{
  ...
  if (gcomp_var[father_cvar].l_operator == MUL_OP)
    {
	  ...
	  (gcomp_var[gcomp_var[father_cvar].second_op].l_operator = FIX_NUMBER))
	{
  ...
  if (gcomp_var[father_cvar].l_operator == MINIMIZE_OP)
...
int insert_ef_in_efconn(EfConn *ef) {
  ...
  if (gextended_ef_block > num_block)
      ...
      if (GpG.numeric_actions)
	{
      ...
      if (GpG.variable_cost_actions)
	{
	  GpG.variable_cost_actions = (int *)realloc(GpG.variable_cost_actions, gextended_ef_block * sizeof(int));
	  memset(&GpG.variable_cost_actions[num_block], 0, (gextended_ef_block - num_block) * sizeof(int)); 
	}
    }
...
void remove_action_from_fact(int action, int fact) {
  ...
  for (i = 0; i < gft_conn[fact].num_PC; i++)
    if (gft_conn[fact].PC[i] == action) {
      gft_conn[fact].PC[i] = gft_conn[fact].PC[--gft_conn[fact].num_PC];
      break;
    }

  for (i = 0; i < gft_conn[fact].num_A; i++)
    if (gft_conn[fact].A[i] == action) {
      gft_conn[fact].A[i] = gft_conn[fact].A[--gft_conn[fact].num_A];
      break;
    }

  for (i = 0; i < gft_conn[fact].num_D; i++)
    if (gft_conn[fact].D[i] == action) {
      gft_conn[fact].D[i] = gft_conn[fact].D[--gft_conn[fact].num_D];
      break;
    }
}


void clean_numeric_preconditions(void) 
{
  ...
  for (i = 0; i < gnum_ef_conn; i++) 
    {
      toDel = FALSE;
      action = &gef_conn[i];
      for (j = 0; j < action->num_PC; j++) 
	{
	  pc = action->PC[j];
	  if ((pc < 0) && (gcomp_var[-pc].l_operator == FIX_NUMBER)) 
	    {
	      if (gcomp_var_value[-pc] < 0.5) {
		action->level = -1; //azione irraggiungibile!
		toDel = TRUE;
	      }
	      else
		action->PC[j--] = action->PC[--action->num_PC]; //elimino questa precondizione
	    }
	}

      if (action->sf != NULL) 
	{
	  for (j = 0; j < action->sf->num_PC_overall; j++) 
	    {
	      pc = action->sf->PC_overall[j];
	      if ((pc < 0) && (gcomp_var[-pc].l_operator == FIX_NUMBER)) 
		{
		  if (gcomp_var_value[-pc] < 0.5) {
		    action->level = -1; //azione irraggiungibile!
		    ...
		    action->sf->PC_overall[j--] = action->sf->PC_overall[--action->sf->num_PC_overall]; //elimino questa precondizione
		}
	    }


	  for (j = 0; j < action->sf->num_PC_end; j++) 
	    {
	      pc = action->sf->PC_end[j];
	      if ((pc < 0) && (gcomp_var[-pc].l_operator == FIX_NUMBER)) 
		{
		  if (gcomp_var_value[-pc] < 0.5) {
		    action->level = -1; //azione irraggiungibile!
		    ...
		    action->sf->PC_end[j--] = action->sf->PC_end[--action->sf->num_PC_end]; //elimino questa precondizione
		}
	    }
	...
	for (j = 0; j < gef_conn[i].num_PC; j++)
	  remove_action_from_fact(i, gef_conn[i].PC[j]);

	for (j = 0; j < gef_conn[i].num_A; j++)
	  remove_action_from_fact(i, gef_conn[i].A[j]);

	for (j = 0; j < gef_conn[i].num_D; j++)
	  remove_action_from_fact(i, gef_conn[i].D[j]);

	if (gef_conn[i].sf != NULL) {
	
	  for (j = 0; j < gef_conn[i].sf->num_PC_overall; j++)
	    remove_action_from_fact(i, gef_conn[i].sf->PC_overall[j]);

	  for (j = 0; j < gef_conn[i].sf->num_PC_end; j++)
	    remove_action_from_fact(i, gef_conn[i].sf->PC_end[j]);

	  for (j = 0; j < gef_conn[i].sf->num_A_start; j++)
	    remove_action_from_fact(i, gef_conn[i].sf->A_start[j]);

	  for (j = 0; j < gef_conn[i].sf->num_D_start; j++)
	    remove_action_from_fact(i, gef_conn[i].sf->D_start[j]);
...
void replace_vars(Fact *f, int *instTable)
{
  int i;
  for (i = 0; i < garity[f->predicate]; i++)
    {
      if (f->args[i] < 0)
	f->args[i] = instTable[DECODE_VAR(f->args[i])];
    }
}


void add_durative_preconditions(int numef)
{
  int j, k, num_pc;
  EfConn *ef;
  OpConn *opconn;
  ...
  Fact *precs;
  Fact tmp_ft;
  Bool to_del[gef_conn[numef].num_PC];
  
  num_pc = 0;
  memset(to_del, 0, gef_conn[numef].num_PC * sizeof(Bool));
  ef = &gef_conn[numef];
  opconn = &gop_conn[ef->op];
  act = opconn->action;
  if (act->norm_operator)
    {
      precs = act->norm_operator->preconds;
      num_pc = act->norm_operator->num_preconds;
    }
  else 
    {
      precs = act->pseudo_action->preconds;
      num_pc =  act->pseudo_action->num_preconds;
    }
  
  for (j = 0; j < num_pc; j++)
    {
      memcpy(&tmp_ft, &precs[j], sizeof(Fact));
      tmp_ft.is_start_end_ovr = 0;
      replace_vars(&tmp_ft, act->inst_table);
      if (precs[j].is_start_end_ovr == IS_AT_END)
	{
	  for (k = 0; k < ef->num_PC; k++)
	    {
	      if (identical_fact(&grelevant_facts[ef->PC[k]], &tmp_ft))
		{
		  ef->sf->PC_end[ef->sf->num_PC_end++] = ef->PC[k];
		  to_del[k] = TRUE; 
		  break;
		}
	    }
	}
      else if (precs[j].is_start_end_ovr == IS_OVERALL)
	{
	  for (k = 0; k < ef->num_PC; k++)
	    {
	      if (identical_fact(&grelevant_facts[ef->PC[k]], &tmp_ft))
		{
		  ef->sf->PC_overall[ef->sf->num_PC_overall++] = ef->PC[k];
		  to_del[k] = TRUE;
		  break;
		}
	    }
  ...
  for (j = 0; j < gef_conn[numef].num_PC; j++)
    {
      if (to_del[j])
	{
	  gef_conn[numef].PC[j--] = gef_conn[numef].PC[--gef_conn[numef].num_PC];
	  to_del[j+1] = to_del[gef_conn[numef].num_PC];
	}
...
void add_durative_effects(int numef) // NON FINITA. DA SISTEMARE!! (fare come in durative_precs con to_del!)
{
  int j, k;
  EfConn *ef;
  OpConn *opconn;
  ...
  if (act->norm_operator)
    {
      adds = act->norm_operator->effects->adds;
      num_A = act->norm_operator->effects->num_adds;
      dels = act->norm_operator->effects->dels;
      num_D = act->norm_operator->effects->num_dels;
    }
  else 
    {
      adds = act->pseudo_action->effects->adds;
      num_A = act->pseudo_action->effects->num_adds;
      dels = act->pseudo_action->effects->dels;
      num_D = act->pseudo_action->effects->num_dels;
    }
  
  for (j = 0; j < num_A; j++)
    {
      memcpy(&tmp_ft, &adds[j], sizeof(Fact));
      tmp_ft.is_start_end_ovr = 0;
      replace_vars(&tmp_ft, act->inst_table);
      if (adds[j].is_start_end_ovr == IS_AT_START)
	{
	  for (k = 0; k < ef->num_A; k++)
	    {
	      if (identical_fact(&grelevant_facts[ef->A[k]], &tmp_ft))
		{
		  ef->sf->A_start[ef->sf->num_A_start++] = ef->A[k];
		  ef->A[k]= ef->A[--ef->num_A];
		  break;
		}
	    }
	}      
   ...
   for (j = 0; j < num_D; j++)
    {
      memcpy(&tmp_ft, &dels[j], sizeof(Fact));
      tmp_ft.is_start_end_ovr = 0;
      replace_vars(&tmp_ft, act->inst_table);
      if (dels[j].is_start_end_ovr == IS_AT_START)
	{
	  for (k = 0; k < ef->num_D; k++)
	    {
	      if (identical_fact(&grelevant_facts[ef->D[k]], &tmp_ft))
		{
		  ef->sf->D_start[ef->sf->num_D_start++] = ef->D[k];
		  ef->D[k]= ef->D[--ef->num_D];
		  break;
		}
	    }
	}      
