  ...
  if (GpG.derived_predicates)
    {
      ...
      //Abilito il pruning se non è stato disabilitato da linea di comando e le regole sono ricorsive
 ...
 */
  ...
  case FAL:
    ...
    /*
     * Un nodo numerico non viene trasformato in WffNode. Verrà gestito in seguito
     **
...

{
  ...
  if ( index == garity[p] ) {
    ...
    /**
     * Aggiungo un fatto allo stato iniziale solo se non è derivato
     **
	...
	if (GET_BIT(rules_precs[j], ef_pred))
	  {
		...
		//Se un assioma usa se stesso allora c'è ricorsione, 
