...
def build_exploration_rules(task):
    result = []
    for proxy in all_conditions(task):
        #print ("Conditions pr type "+str(type(proxy)))
        proxy.build_rules(result)
        #print ("Conditions Results "+str(result))
    return result
...
def condition_to_rule_body(parameters, condition):
    ...
    if not isinstance(condition, pddl.Truth):
        ...
        for part in parts:
            ...
            #print ("Conditions pr type "+str(condition.parts))
            assert isinstance(part, pddl.Literal), "Condition not normalized: %s %s" % (condition.parts,str(pddl.Literal))
            if not part.negated:
                result.append(part)
    return result
