...
class Action(object):
    ...
    def instantiate_mini(self, var_mapping):
        """
            A reduced instantiation function, that can produce mapping even for impossible actions
        """
        arg_list = [var_mapping[par.name]
                    for par in self.parameters[:self.num_external_parameters]]
        name = "(%s %s)" % (self.name, " ".join(arg_list))
        effects = []
        precondition = []
        self.precondition.instantiate_mini(var_mapping,precondition)
        for eff in self.effects:
            eff.instantiate_mini(var_mapping,effects)
