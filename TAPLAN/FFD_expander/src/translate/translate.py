...
def translate_task(strips_to_sas, ranges, translation_key,
    ...
    #with timers.timing("Processing init", block=True):
    init = sas_tasks.SASInit(init_values)

    #with timers.timing("Processing goal", block=True):
    ...
    #with timers.timing("Processing ops", block=True):
    operators = translate_strips_operators(actions, strips_to_sas, ranges,
                                               mutex_dict, mutex_ranges,
                                               implied_facts)

    #with timers.timing("Translating axioms", block=True):
...
def translate_task_lite(strips_to_sas, ranges, translation_key,
                   mutex_dict, mutex_ranges, mutex_key,
                   init, goals,
                   actions, axioms, metric, implied_facts,operators):
    axioms, axiom_init, axiom_layer_dict = axiom_rules.handle_axioms(
            actions, axioms, goals)
    init = init + axiom_init
    #axioms.sort(key=lambda axiom: axiom.name)
...
def translate_task_templ(strips_to_sas, ranges, translation_key,
                   mutex_dict, mutex_ranges, mutex_key,
                   init,
                   actions, axioms, metric, implied_facts):
    #with timers.timing("Processing ops", block=True):
...
def pddl_to_sas_lite(task,actions,relaxed_reachable, atoms,axioms,reachable_action_params):
...
#            if eff_pred.literal.predicate not in private_pred_list:
    ...
    return task,actions,list(IGNORE_ACTIONS),relaxed_reachable, atoms,axioms,reachable_action_params


def create_sas_file(task,inst_actions,relaxed_reachable, atoms,axioms,reachable_action_params,op_filename):
    sas_task = pddl_to_sas_lite(task,inst_actions,relaxed_reachable, atoms,axioms,reachable_action_params)
    #print ("Goals pair "+str(sas_task.goal.pairs))
    with open(op_filename, "w") as output_file:
