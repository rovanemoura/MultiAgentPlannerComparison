...
class PlanningProblem(object):
  ...
  def parse_domain(self, domainfile):
    ...
    for action in self.actions:
      ...
      avar, atype = agent[0] 
      new_act = Action(act_name, atype, up_params, pre_list, eff_list)
      #print (agent)
  ...
  def compile_to_TA_problem(self):
    ...
    agent_flag = {} # helps identify if there are any agents in the given list which can be better classified as a resource
    #agent_types = set()
    #print (self.agent_types)
    for ag in self.agent_types:
        agent_flag[ag] = False
    for act in self.actions:
        parameter_list = []
        for v,p in act.parameters.args:
            if p != act.agent:
                parameter_list.append(p)
        if len(set(parameter_list) & self.agent_types) == 0:
            agent_flag[act.agent] = True
    print (agent_flag)
    for ag in self.agent_types:
        if agent_flag[ag]:
            if ag in self.types.keys():
                _types = self.types[ag]
            else:
                 _types = [ag]
            for ty in _types:
                self.TA_agents.extend(self.objects[ty])
    #agent_list =  list()
    TA_NAME = self.TA_agents[0]
    #print (self.agents)
    # objects
