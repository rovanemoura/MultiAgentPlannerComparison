...
void initsymboltable() {
  ...
  maxSymbols = MAXBUCKETS;
  index2stentry = (stentry **)malloc(maxSymbols * sizeof(stentry *));
}

/* Hash function for strings (PDDL symbols) */

int symbolhash1(char *s) {
  int hash,c,shift;
  shift = 0;
  hash = 0;
  while(*s != 0) {
...
int symbolindex(char *s) {
  ...
  ns = (char *)malloc(strlen(s)+1); /* Make a new copy of the string */
  strcpy(ns,s);
  ...
  if(ste->index == -1) {
    ...
    //    printf("[%i/%i]\n",tmp,nOfSymbols);
    ste->next = (stentry *)malloc(sizeof(stentry));
    ste->next->s = ns;
...
char *symbol(int i) {
  ...
  case -3: return "PARAM-3";
  case -4: return "PARAM-4";
  case -5: return "PARAM-5";
  case -6: return "PARAM-6";
  default:
...
#define ATABLESIZE 0x80000
#define MAXARITY 10

typedef struct _atomhashelement{
  int index;
  intlist *a;
  struct _atomhashelement *tl;
} atomhashelement;

atomhashelement *atomhash[ATABLESIZE];
...
void initatomtable() {
  ...
  atomtable = (intlist **)malloc(sizeof(intlist *) * maxAtoms);
...
int atomindex(atom *a,int *b) {
  ...
  l = (atomhashelement *)malloc(sizeof(atomhashelement));
