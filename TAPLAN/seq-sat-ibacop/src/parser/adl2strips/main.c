...

{
  ...
  if ( gcmd_line.strips ) {
    printf("\n\nputting out grounded STRIPS task, %d facts\ninto ``domain_wce.txt'' and ``facts_wce.txt''", gnum_ft_conn);
    ...
    printf("\n\nputting out grounded SIMPLE-ADL task\ninto ``domain_wce.txt'' and ``facts_wce.txt''\n\n");
    fflush( stdout );
...

{
  ...
  if ( !a->norm_operator &&
    ...
    for ( i = 0; i < a->num_name_vars; i++ ) {
      fprintf(out, "_---_%s", gconstants[a->name_inst_table[i]]);
    }
  }
...

{
  ...
  printf("-s <str>    output path for operator and fact file\n");
...

{
  ...
  memset(gcmd_line.output_path, 0, MAX_LENGTH);
