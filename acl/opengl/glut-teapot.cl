;; $Id: glut-teapot.cl,v 1.1.1.1 2006/08/15 18:21:14 layer Exp $
;;
;; This is the Glut version of the teapot.
...
(defun convert-c-array (string &key (pos 0) (max (length string)) type)
  ...
  (let ((char #\,)
    ...
    (while (char/= char #\})
      ...
      (if* (char= char #\{)
	 then (multiple-value-bind (array newpos)
		  (convert-c-array string :pos pos :max max :type type)
		(setf pos newpos)
		(push array res))
	      ;; pos points to next non-whitespace char after the closing
	      ;; brace.  which should be a comma or closing brace.
	 else (let ((startpos pos))
...
(defparameter rotX 0.0)
(defparameter rotY 0.0)
(defparameter teaList nil)

(ff:defun-foreign-callable Draw ()
...
(ff:defun-foreign-callable Reshape ((w :long) (h :long))
  (glViewport 0  0 w h)
...
(ff:defun-foreign-callable Key ((key :unsigned-char) (x :long) (y :long))
  (if (= key 27)
      (exit 0)))
      

(ff:defun-foreign-callable SpecialKey ((key :long) (x :long) (y :long))
  (case key
    (#.GLUT_KEY_UP
     (decf rotX 20.0))
    (#.GLUT_KEY_DOWN
...
(defun teapot (&rest args)
  (let ((argcp (ff:allocate-fobject '(* :int) :c))
	(argv (ff:allocate-fobject '(* (* :char)) :c)))
    (setf (ff:fslot-value-typed '(* :int) :c argcp) 0)
    (setf (ff:fslot-value-typed '(* (* :char)) :c argv) 0)
    (glutInit argcp argv))

  (let ((type (logior GLUT_RGB GLUT_DEPTH)))
    (glutInitDisplayMode type)
    (glutInitWindowSize 300 300)
    (with-native-string (ts "Teapot")
      (glutCreateWindow ts))
    
    (Init)

    (let ((reshape-callback (ff:register-foreign-callable 'Reshape :reuse t))
	  (key-callback (ff:register-foreign-callable 'Key :reuse t))
	  (special-key-callback
	   (ff:register-foreign-callable 'SpecialKey :reuse t))
	  (draw-callback (ff:register-foreign-callable 'Draw :reuse t)))
