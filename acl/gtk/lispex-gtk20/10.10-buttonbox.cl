...
(defun create-bbox (horizontal title spacing child-w child-h layout)
  (declare (ignore child-w child-h))
  (let ((frame nil)
	(bbox nil)
	(button nil))
    ...
    (gtk:gtk_box_set_spacing (gtk:GTK_BOX bbox) spacing)

    (setq button (gtk:gtk_button_new_from_stock gtk:GTK_STOCK_OK))
    (gtk:gtk_container_add (gtk:GTK_CONTAINER bbox) button)

    (setq button (gtk:gtk_button_new_from_stock gtk:GTK_STOCK_CANCEL))
    (gtk:gtk_container_add (gtk:GTK_CONTAINER bbox) button)

    (setq button (gtk:gtk_button_new_from_stock gtk:GTK_STOCK_HELP))
    (gtk:gtk_container_add (gtk:GTK_CONTAINER bbox) button)
    frame))
