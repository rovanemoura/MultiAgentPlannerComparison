...
(defpackage "09.11-calendar" (:use :excl :common-lisp))
(in-package "09.11-calendar")
...
(defun calendar-set-signal-strings (sig-str data)
  (let ((prev-sig nil))
    ...
    (setq prev-sig
      (gtk:gtk_label_get_text (gtk:GTK_LABEL (calendar-prev-sig data))))
    (gtk:gtk_label_set_text (gtk:GTK_LABEL (calendar-prev2-sig data))
			    prev-sig)

    (setq prev-sig
      (gtk:gtk_label_get_text (gtk:GTK_LABEL (calendar-last-sig data))))
    (gtk:gtk_label_set_text (gtk:GTK_LABEL (calendar-prev-sig data))
			    prev-sig)
    (gtk:gtk_label_set_text (gtk:GTK_LABEL (calendar-last-sig data))
			    sig-str)))
...
(ff:defun-foreign-callable calendar-font-selection-ok ((button
  ...
  (let ((style nil)
    ...
    (when (not (eql gtk:NULL (calendar-window calendar)))
      (setq font-desc (gtk:pango_font_description_from_string
		       (calendar-font calendar)))
      (when (not (eql gtk:NULL font-desc))
	(setq style (gtk:gtk_style_copy
		     (gtk:gtk_widget_get_style (calendar-window calendar))))
	(setf (ff:fslot-value-typed 'gtk:GtkStyle nil style 'gtk::font_desc)
	  font-desc)
...
(ff:defun-foreign-callable calendar-select-font ((button (* gtk:GtkWidget))
  ...
  (let ((calendar (ff:lisp-value calendar-cb))
	...
	window)
      ...
      (gtk:gtk_window_set_position (gtk:GTK_WINDOW window)
				   gtk:GTK_WIN_POS_MOUSE)
      ...
      (gtk:g_signal_connect
       (gtk:G_OBJECT (ff:fslot-value-typed
...
(let ((calendar-data (ff:register-lisp-value (make-instance 'CalendarData))))
  (defun create-calendar ()
	...
	(setf (aref (calendar-settings (ff:lisp-value calendar-data)) i) nil))
      ...
      (gtk:g_signal_connect (gtk:G_OBJECT window) "destroy"
			    (gtk:G_CALLBACK
			     ...
			     #-original (ff:register-foreign-callable
					 'cb-gtk-main-quit))
			    gtk:NULL)
      (gtk:g_signal_connect (gtk:G_OBJECT window) "delete-event"
			    (gtk:G_CALLBACK
			     (ff:get-entry-point "gtk_false"))
			    gtk:NULL)

      (gtk:gtk_window_set_resizable (gtk:GTK_WINDOW window) gtk:FALSE)
      ...
      (gtk:gtk_box_set_spacing (gtk:GTK_BOX hbbox) 5)
      ...
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "month_changed"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-month-changed)) 
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "day_selected"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-day-selected))
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar)
			    "day_selected_double_click"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-day-selected-double-click) )
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "prev_month"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-prev-month))
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "next_month"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-next-month))
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "prev_year"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-prev-year))
			    calendar-data)
      (gtk:g_signal_connect (gtk:G_OBJECT calendar) "next_year"  
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'calendar-next-year))
			    calendar-data)
	...
	(gtk:g_signal_connect (gtk:G_OBJECT toggle) "toggled" 
			      (gtk:G_CALLBACK
			       (ff:register-foreign-callable
...
(flet ((run-example (name function)
	 ...
	 (when gtk::*run-example*
	   (mp:process-run-function
	    ...
	    function))))
  (run-example "09.11-calendar" #'calendar))
