...
(defpackage "16.03-setselection" (:use :excl :common-lisp))
(in-package "16.03-setselection")
...
(let ((have-selection (ff:allocate-fobject 'gtk:gint
  ...
  (defun setselection ()
    (let ((window nil)
      ...
      (gtk:g_signal_connect (gtk:G_OBJECT selection-button) "toggled" 
			    (gtk:G_CALLBACK
			     (ff:register-foreign-callable
			      'selection-toggled))
			    have-selection)
      (gtk:g_signal_connect (gtk:G_OBJECT selection-button)
...
(flet ((run-example (name function)
	 ...
	 (when gtk::*run-example*
	   (mp:process-run-function
	    ...
	    function))))
  (run-example "16.03-setselection" #'setselection))
