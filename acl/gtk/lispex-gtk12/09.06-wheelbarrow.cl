...
(defpackage "09.06-wheelbarrow" (:use :excl :common-lisp))
(in-package "09.06-wheelbarrow")
...
(defparameter WheelbarrowFull-xpm
...
(defun wheelbarrow ()
  (let ((window nil)
    ...
    (gtk:gtk_init 0 0)
    (setq window (gtk:gtk_window_new gtk:GTK_WINDOW_POPUP))
    ...
    (setq style (gtk:gtk_widget_get_default_style))
    (setq gc (ff:fslot-value-typed 'gtk:GtkStyle nil style 'gtk::black_gc))
    ;; So that we can pass &mask to gdk_pixmap_create_from_xpm
    (setq amask (ff:allocate-fobject '(* gtk:GdkBitmap)
				     :foreign-static-gc))
    (setq gdk_pixmap (gtk:gdk_pixmap_create_from_xpm_d
		      ;; window->window
		      (ff:fslot-value-typed 'gtk:GtkWidget nil window
					    'gtk::window)
    ...
    (setq pixmap (gtk:gtk_pixmap_new gdk_pixmap mask))
    (gtk:gtk_widget_show pixmap)

    (setq fixed (gtk:gtk_fixed_new))
    (gtk:gtk_widget_set_usize fixed 200 200)
    (gtk:gtk_fixed_put (gtk:GTK_FIXED fixed) pixmap 0 0)
    (gtk:gtk_container_add (gtk:GTK_CONTAINER window) fixed)
    (gtk:gtk_widget_show fixed)

    (gtk:gtk_widget_shape_combine_mask window mask 0 0)

    (gtk:gtk_widget_set_uposition window 20 400)
    (gtk:gtk_widget_show window)
...
(flet ((run-example (name function)
	 ...
	 (when gtk::*run-example*
	   (mp:process-run-function
	    ...
	    function))))
  (run-example "09.06-wheelbarrow" #'wheelbarrow))
