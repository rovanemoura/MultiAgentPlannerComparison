...
(defpackage "14.04-itemfactory" (:use :excl :common-lisp))
(in-package "14.04-itemfactory")
...
(defun itemfactory ()
  (let ((window nil)
    ...
    (gtk:gtk_widget_set_usize (gtk:GTK_WIDGET window) 300 200)

    (setq main-vbox (gtk:gtk_vbox_new gtk:FALSE 1))
    (gtk:gtk_container_border_width (gtk:GTK_CONTAINER main-vbox) 1)
...
(flet ((run-example (name function)
	 ...
	 (when gtk::*run-example*
	   (mp:process-run-function
	    ...
	    function))))
  (run-example "14.04-itemfactory" #'itemfactory))
